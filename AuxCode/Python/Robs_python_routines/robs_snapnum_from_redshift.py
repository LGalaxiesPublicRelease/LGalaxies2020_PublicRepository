...
##########
def robs_snapnum_from_redshift(sim, cos, redshifts, decimal_places=2) :
    # Convert singluar float or integer snaps into list, if necessary:
    if (isinstance(redshifts, float)) | (isinstance(redshifts, int)) :
        redshifts = list([redshifts])
    
    if (sim == 'Mil-II') :
        if (cos == 'Planck') :
            All_redshifts = np.array([56.418465,49.24166,37.83254,29.547876,26.951857,24.604391,22.4798,20.555162,\
        ...
        if (cos == 'Planck') :
            All_redshifts = np.array([56.418465, 49.24166, 44.754864, 26.812078, 17.76479, 16.264746, 14.901791, 13.662391,\
    ...
    for iz in range(0,len(redshifts)) :
        ...
        snapnums[iz] = np.abs(All_redshifts - redshifts[iz]).argmin()
