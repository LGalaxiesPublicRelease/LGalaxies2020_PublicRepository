...
# Flag to decide whether or not this entry contains a metal channels array
    flagMetals[n]=match(name[n],/\[NUM_METAL_CHANNELS\]/)
    if (flagMetals[n]>0) flagMetals[n]=1
...
END{
    ...
    print "int flagMetals[]={"
    for(i=0;i<n;i++) print flagMetals[i] ","
    print "}; "
