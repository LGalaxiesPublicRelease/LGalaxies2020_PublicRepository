...
void deal_with_satellites(int ngal)
{
  ...
  for(i = 0; i < ngal; i++) {   /* Loop over all galaxies in the FoF-halo */
	  for (int ee=0;ee<NUM_ELEMENTS;ee++)
		  if (Gal[i].EjectedMass_elements[ee] < 0.0)
			  printf("IN deal_with_satellites(): Gal[%i].EjectedMass_elements[%i] = %e | Gal[%i].HotGas_elements[%i] = %e\n",
	  				i, ee, Gal[i].EjectedMass_elements[ee], i, ee, Gal[i].HotGas_elements[ee]);
      ...
      if(HotGasStripingModel == 0) {
    	  ...
    	  if (Gal[i].Type ==2 && HotGasOnType2Galaxies==0) {
...
#ifdef DETAILED_DUST
    		  if(Gal[i].HotGas > 0.0)
    			  transfer_material(centralgal,"HotGas",i,"HotGas",gasfraction_intotype1,0.0,gasfraction_intotype1,"model_stripping.c", __LINE__);
    		  if(Gal[i].EjectedMass > 0.0) {
    			  //transfer_material(centralgal,"EjectedMass",i,"EjectedMass",gasfraction_intotype1,0.0,0.0,"model_stripping.c", __LINE__);
    			  //Change for EjectaDust:
    			  transfer_material(centralgal,"EjectedMass",i,"EjectedMass",gasfraction_intotype1,0.0,gasfraction_intotype1,"model_stripping.c", __LINE__);
    		  }
#ifdef EXCESS_MASS
    		  if(Gal[i].ExcessMass > 0.0)
    			  transfer_material(centralgal,"ExcessMass",i,"ExcessMass",gasfraction_intotype1,0.0,0.0,"model_stripping.c", __LINE__);
#endif
    		  mass_checks(i,"model_stripping.c",__LINE__);
    		  mass_checks(centralgal,"model_stripping.c",__LINE__);
#ifdef TRACK_BURST
    		  ...
    		  transfer_material(centralgal,"BurstMass",i,"BurstMass",
    				  	  	  	GasFraction_intotype1*Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM),0.0,0.0,"model_stripping.c", __LINE__);
#endif
    		  transfer_material(centralgal,"ICM",i,"ICM",gasfraction_intotype1,0.0,0.0,"model_stripping.c", __LINE__);
#else //DETAILED_DUST
    		  if(Gal[i].HotGas > 0.0)
    		  ...
    		  /* Transfer burst component first */
    		  transfer_material(centralgal,"BurstMass",i,"BurstMass",
    				  	  	  	GasFraction_intotype1*Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM),"model_stripping.c", __LINE__);
#endif
...
#ifdef DETAILED_DUST
    			  if(Gal[i].HotGas > 0.0) {
    				  //transfer_material(FOF_centralgal,"HotGas",i,"HotGas",1.,0.0,1.0,"model_stripping.c", __LINE__);
    				  //Change for EjectaDust:
    				  transfer_material(FOF_centralgal,"HotGas",i,"HotGas",1.,0.,1.,"model_stripping.c", __LINE__);
    			  }
    			  if(Gal[i].EjectedMass > 0.0) {
    				  //transfer_material(FOF_centralgal,"EjectedMass",i,"EjectedMass",1.,0.0,0.0,"model_stripping.c", __LINE__);
    				  //Change for EjectaDust:
    				  transfer_material(FOF_centralgal,"EjectedMass",i,"EjectedMass",1.,0.,1.,"model_stripping.c", __LINE__);
    			  }
#ifdef EXCESS_MASS
    			  if(Gal[i].ExcessMass > 0.0)
    				  transfer_material(FOF_centralgal,"ExcessMass",i,"ExcessMass",1.,0.0,0.0,"model_stripping.c", __LINE__);
#endif
#ifdef TRACK_BURST
    			  /* Transfer burst component first */
    			  transfer_material(FOF_centralgal,"BurstMass",i,"BurstMass",
    					  	  	    Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM),0.0,0.0, "model_stripping.c", __LINE__);
#endif
    			  transfer_material(FOF_centralgal,"ICM",i,"ICM",1.,0.0,0.0, "model_stripping.c", __LINE__);
#else //DETAILED_DUST
    			  if(Gal[i].HotGas > 0.0)
    	  ...
    	  else //Type 1 galaxies (or type 2's for the modified stripping) - stripping if galaxy inside Rvir of stripping_centralgal
			  if(((Gal[i].Type==1 && dis < Gal[FOF_centralgal].Rvir) || (Gal[i].Type==2 && dis < Gal[centralgal].Rvir && HotGasOnType2Galaxies==1)) && Gal[i].HotGas > 0.0 ) {
				  if(Gal[i].Type == 1)
...
#ifdef DETAILED_DUST
				  transfer_material(stripping_centralgal,"HotGas",i,"HotGas",stripped_fraction,0.0,stripped_fraction,"model_stripping.c", __LINE__);
				  //transfer_material(stripping_centralgal,"EjectedMass",i,"EjectedMass",stripped_fraction,0.0,0.0,"model_stripping.c", __LINE__);
				  //Change for EjectaDust:
				  transfer_material(stripping_centralgal,"EjectedMass",i,"EjectedMass",stripped_fraction,0.0,stripped_fraction,"model_stripping.c", __LINE__);
#ifdef EXCESS_MASS
				  transfer_material(stripping_centralgal,"ExcessMass",i,"ExcessMass",stripped_fraction,0.0,0.0,"model_stripping.c", __LINE__);
#endif
				  mass_checks(i,"model_stripping.c",__LINE__);
				  mass_checks(stripping_centralgal,"model_stripping.c",__LINE__);
#ifdef TRACK_BURST
				  ...
				  transfer_material(stripping_centralgal,"BurstMass",i,"BurstMass",
						  	  	    stripped_fraction*Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM),0.0,0.0,"model_stripping.c", __LINE__);
#endif
				  transfer_material(stripping_centralgal,"ICM",i,"ICM",stripped_fraction,0.0,0.0,"model_stripping.c", __LINE__);
#else //DETAILED_DUST
				  ...
				  /* Transfer burst component first */
				  transfer_material(stripping_centralgal,"BurstMass",i,"BurstMass",
						  	  	    stripped_fraction*Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM), "model_stripping.c", __LINE__);
#endif
	  ...
	  if ( dis < Gal[FOF_centralgal].Rvir && i != FOF_centralgal) {
	      /* Note: the original code transferred from ejected to hot here, but had all other
...
#ifdef DETAILED_DUST
	      //transfer_material(centralgal,"HotGas",i,"HotGas",1.,0.0,1.0,"model_stripping.c", __LINE__);
	      //transfer_material(centralgal,"EjectedMass",i,"EjectedMass",1.,0.0,0.0,"model_stripping.c", __LINE__);
	      //Change for EjectaDust:
	      transfer_material(centralgal,"HotGas",i,"HotGas",1.,0.,1.,"model_stripping.c", __LINE__);
	      transfer_material(centralgal,"EjectedMass",i,"EjectedMass",1.,0.,1.,"model_stripping.c", __LINE__);
#ifdef EXCESS_MASS
	      transfer_material(centralgal,"ExcessMass",i,"ExcessMass",1.,0.0,0.0,"model_stripping.c", __LINE__);
#endif
#ifdef TRACK_BURST
	      /* Transfer burst component first */
	      transfer_material(centralgal,"BurstMass",i,"BurstMass",
				Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM),0.0,0.0,"model_stripping.c", __LINE__);
#endif
	      transfer_material(centralgal,"ICM",i,"ICM",1.,0.0,0.0,"model_stripping.c", __LINE__);
#else //DETAILED_DUST
