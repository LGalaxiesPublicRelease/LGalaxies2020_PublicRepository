...
double get_likelihood()
{
  ...
  double logfinal_prob, logredshift_prob, logcurrent_prob;
...
******************************/
        if(strcmp(MCMC_Obs[ObsNr].TestType,"chi_sq")==0)
        {
          ...
          logcurrent_prob=log10(chi_square_probability(ObsNr, binsamdata, snap));
 ...
 ******************************/
	      if(strcmp(MCMC_Obs[ObsNr].TestType,"maxlike")==0)
        {
          ...
          logcurrent_prob=log10(maximum_likelihood_probability(ObsNr, binsamdata, snap));
 ...
 ******************************/
	      if(strcmp(MCMC_Obs[ObsNr].TestType,"binomial")==0)
        {
          if(strcmp(MCMC_Obs[ObsNr].Name,"BlackHoleBulgeMass")==0)
          {
            ...
            logcurrent_prob=log10(binomial_probability(ObsNr, binblackholeup, binblackholedown, snap));
          }
        }
//END BINOMIAL TESTS
...
//#endif
        ...
        printf("= %0.5e\n",ObsNr,pow(10.,logcurrent_prob));
	      logredshift_prob+=logcurrent_prob;
	      //write likelihood for each constraint at each MCMC step
	      fprintf(FILE_MCMC_PredictionsPerStep[snap][ObsNr], " %0.5e", pow(10.,logcurrent_prob));
    ...
    printf("Likelihood snap[%d]=%0.5e (log: %0.8g)\n\n",snap, pow(10.,logredshift_prob), logredshift_prob);

    logfinal_prob+=logredshift_prob;
    free(PropertyToBin);
#ifdef HALOMODEL
    free(MCMC_FOF2);
    free(HashTable);
  ...
  printf("FINAL LIKELIHOOD=%0.5e\nlog_like=%0.8g\n",pow(10.,logfinal_prob), -logfinal_prob);
