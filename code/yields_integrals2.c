				  ...
				  for (Zi=0;Zi<LIFETIME_Z_NUM;Zi++) {
					  ...
					  */
					  ...
					  if (t_upper >= lifetimes[Zi][LIFETIME_MASS_NUM-1]) { //If the longest time from SFH bin i to current timestep is shorter than the shortest possible lifetime, there's no enrichment, so skip calculations.
						  ...
						  Mi_lower_lastTS[i][Zi][0] = Mi_upper_actual; //DEFAULT MODE: Uncomment this to go back to the old mode, with mass over/underlaps.
						  ...
						  if (Mi_lower_lt_SNII <= Mi_upper_lt_SNII) {
							  ...
							  for (j=Mi_lower_SNII;j<=Mi_upper_SNII;j++) {
								  ...
								  //**********
#ifdef INDIVIDUAL_ELEMENTS
								  calc_mass_limits_SNII(j, kk, Zi_SNII, Mi_lower_SNII, Mi_upper_SNII, Mi_lower_actual, Mi_upper_actual,
						  ...
						  if ((lifetimes[Zi_correc][Mi_lower] > SNIA_MIN_TIME) && (lifetimes[Zi_correc][Mi_upper] < SNIA_MAX_TIME) && (t_upper > SNIA_MIN_TIME) && (t_lower < SNIA_MAX_TIME)) {
							  int j;
...
#endif //INDIVIDUAL_ELEMENTS
{
	...
	//4) If mass bin j IS the lowest but IS NOT the highest mass bin to be integrated over:
	//This will work for either (SNIIMasses[j] <= SNIA_MAX_MASS) or (SNIIMasses[j] > SNIA_MAX_MASS)
	else if (j == Mi_lower_SNII && j != Mi_upper_SNII && Mi_lower_actual >= SNII_MIN_MASS) {
		*ML = Mi_lower_actual;
		*MU = SNIIMasses[j+1];
	...
	//6) If mass bin j IS NOT the lowest but IS the highest mass bin to be integrated over:
	//This will work for either (SNIIMasses[j] <= SNIA_MAX_MASS) or (SNIIMasses[j] > SNIA_MAX_MASS)
