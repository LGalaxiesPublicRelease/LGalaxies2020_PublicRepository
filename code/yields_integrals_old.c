...
void init_integrated_yields()
{
  ...
  for(ii=0;ii<STEPS*LastDarkMatterSnapShot;ii++)
      ...
      for(kk=0;kk<LIFETIME_Z_NUM;kk++)
	{
	  ...
	  NormSNIaMassEjecRate2[ii][jj][kk]=0.;
	  NormSNIaMassEjecRateOld[ii][jj][kk]=0.;
	  NormSNIaMetalEjecRate2[ii][jj][kk]=0.;
#ifdef INDIVIDUAL_ELEMENTS
	  for(ll=0;ll<NUM_ELEMENTS;ll++) {
	    NormSNIaYieldRate[ii][jj][kk][ll]=0.;
	    NormSNIaYieldRate2[ii][jj][kk][ll]=0.; }
#endif
	  NormSNIINum[ii][jj][kk]=0.;
	  NormSNIaNum[ii][jj][kk]=0.;
	  NormSNIaNum2[ii][jj][kk]=0.;
		  ...
		  for (Zi=0;Zi<LIFETIME_Z_NUM;Zi++) //LOOP OVER POSSIBLE INITIAL METALLICITIES
		      ...
		      if (t_upper >= lifetimes[Zi][LIFETIME_MASS_NUM-1]) //If the longest time from SFH bin i to current timestep is shorter than the shortest possible lifetime, there's no enrichment, so skip calculations.
			{
			  ...
			  if ((lifetimes[Zi][Mi_lower] > SNIA_MIN_TIME) && (lifetimes[Zi][Mi_upper] < SNIA_MAX_TIME) && (t_upper > SNIA_MIN_TIME) && (t_lower < SNIA_MAX_TIME))
				...
				{
			    	  NormSNIaMassEjecRateOld[(STEPS*snap)+step][i][Zi] = NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi];
				...
				int k; //Iterator for the L-Galaxies elements arrays [0 to 10]
				  ...
				  //if (snap == 2 && step < 10) printf("\nOld: %i %i %i %i %i %i | NormSNIaMassEjecRate[%i][%i][%i] =     %f\n", snap, step, i, mb, Zi, j, (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi]);
				  //if (snap == 2 && step < 10) printf("     added: %.12e\n", NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi] - NormSNIaMassEjecRateOld[(STEPS*snap)+step][i][Zi]);
				  //*****
				  //New version: start
				  //*****
				  ...
				  double DTD_lower2, DTD_upper2, lifetimes_lower, lifetimes_upper;
				  calc_time_limits_SNIa(j, Zi_correc, t_lower_lifetime, t_upper_lifetime, t_lower, t_upper,
						  	  	  	    &lifetimes_lower, &lifetimes_upper, &stat);
				  if (lifetimes_lower == 0.0 || lifetimes_upper == 0.0) {
					  DTD_lower2 = 0.0;
					  DTD_upper2 = 0.0;
				  }
				  else {
					  DTD_lower2 = DTDcalc(lifetimes_lower) * 1.0e-9; //NOTE: DTDcalc returns SNe/Gyr, not SNe/yr, hence the multiple (1.0e-9).
					  DTD_upper2 = DTDcalc(lifetimes_upper) * 1.0e-9;
				  }
				  SNIa_Rate2[(STEPS*snap)+step][Zi] += A_FACTOR * F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * ((DTD_lower2 + DTD_upper2) * TheSFH[i]/First_SFH_bin_width)/2.0;
				  NormSNIaNum2[(STEPS*snap)+step][i][Zi] += A_FACTOR * F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * (DTD_lower2 + DTD_upper2)/2.0; //Number of SNe-Ia exploding in this timestep from minibin mb of SFHBin i [units: # / Msun]
				  NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi] += A_FACTOR * F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * ((DTD_lower2 + DTD_upper2)*SNIAEJECMASS)/2.0; //IN [Msun/yr]
				  NormSNIaMetalEjecRate2[(STEPS*snap)+step][i][Zi] = NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi];
#ifdef INDIVIDUAL_ELEMENTS
				  int k;
				  for (k=0;k<NUM_ELEMENTS;k++) {
					  int kk; //Iterator for the SNIa input yield arrays [0 to 41]
#ifndef MAINELEMENTS
					  switch(k){case 0: kk=0; break; case 1: kk=1; break; case 2: kk=5; break; case 3: kk=6; break; case 4: kk=7; break; case 5: kk=9; break; case 6: kk=11; break; case 7: kk=13; break; case 8: kk=15; break; case 9: kk=19; break; case 10: kk=25; break;}
					  NormSNIaYieldRate2[(STEPS*snap)+step][i][Zi][k] += A_FACTOR*F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * ((DTD_lower2 + DTD_upper2) * SNIaYields[kk])/2.0; //IN [Msun/yr]
#else
					  switch(k){case 0: kk=0; break; case 1: kk=1; break; case 2: kk=7; break; case 3: kk=11; break; case 4: kk=25; break;}
					  NormSNIaYieldRate2[(STEPS*snap)+step][i][Zi][k] += A_FACTOR*F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * ((DTD_lower2 + DTD_upper2) * SNIaYields[kk])/2.0; //IN [Msun/yr]
#endif //MAINELEMENTS
...
#endif //INDIVIDUAL_ELEMENTS
				  //if (snap == 2 && step < 10) printf("%i %i %i %i %i %i | SNIa_Rate[%i][%i] = %f | NormSNIaMassEjecRate[%i][%i][%i] = %f\n", snap, step, i, mb, Zi, j, (STEPS*snap)+step, Zi, SNIa_Rate[(STEPS*snap)+step][Zi], (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi]);
				  //if (snap == 2 && step < 10) printf("New: %i %i %i %i %i %i | %i | NormSNIaMassEjecRate[%i][%i][%i] = %f\n", snap, step, i, mb, Zi, j, stat, (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi]);
				  //if (NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi] < 0.0 || NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi] < 0.0) printf("New: %i %i %i %i %i %i | %i | NormSNIaMassEjecRate[%i][%i][%i] = %f\n", snap, step, i, mb, Zi, j, stat, (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi]);
				  //if (snap == 2 && step < 10) printf("     added: %.12e\n", A_FACTOR * F316 * KALPHA * (lifetimes_upper-lifetimes_lower) * ((DTD_lower2 + DTD_upper2)*SNIAEJECMASS)/2.0);
				  //if (NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi] != NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi]) printf("***** WARNING!: NormSNIaMassEjecRate[%i][%i][%i] = %.12e | NormSNIaMassEjecRate2[%i][%i][%i] = %.12e *****\n", (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi], (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi]);
				  //if (NormSNIaMetalEjecRate[(STEPS*snap)+step][i][Zi] - NormSNIaMetalEjecRate2[(STEPS*snap)+step][i][Zi] > 1.e-12 || NormSNIaMetalEjecRate2[(STEPS*snap)+step][i][Zi] - NormSNIaMetalEjecRate[(STEPS*snap)+step][i][Zi] > 1.e-12) printf("***** WARNING!: NormSNIaMetalEjecRate[%i][%i][%i] = %.12e | NormSNIaMetalEjecRate2[%i][%i][%i] = %.12e *****\n", (STEPS*snap)+step, i, Zi, NormSNIaMetalEjecRate[(STEPS*snap)+step][i][Zi], (STEPS*snap)+step, i, Zi, NormSNIaMetalEjecRate2[(STEPS*snap)+step][i][Zi]);
				  //if (NormSNIaYieldRate[(STEPS*snap)+step][i][Zi][0] - NormSNIaYieldRate2[(STEPS*snap)+step][i][Zi][0] > 1.e-12 || NormSNIaYieldRate2[(STEPS*snap)+step][i][Zi][0] - NormSNIaYieldRate[(STEPS*snap)+step][i][Zi][0] > 1.e-12) printf("***** WARNING!: NormSNIaYieldRate[%i][%i][%i][0] = %.12e | NormSNIaYieldRate2[%i][%i][%i][0] = %.12e *****\n", (STEPS*snap)+step, i, Zi, NormSNIaMetalEjecRate[(STEPS*snap)+step][i][Zi], (STEPS*snap)+step, i, Zi, NormSNIaYieldRate2[(STEPS*snap)+step][i][Zi][0]);
				  if (NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi] < 0.0 || NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi] < 0.0) {
					  printf("\nOld: %i %i %i %i %i %i | NormSNIaMassEjecRate[%i][%i][%i] =     %.7e\n", snap, step, i, mb, Zi, j, (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi]);
					  printf("New: %i %i %i %i %i %i | %i | NormSNIaMassEjecRate[%i][%i][%i] = %.7e | lifetimes_lower = %.7e | lifetimes_upper = %.7e\n", snap, step, i, mb, Zi, j, stat, (STEPS*snap)+step, i, Zi, NormSNIaMassEjecRate2[(STEPS*snap)+step][i][Zi], lifetimes_lower, lifetimes_upper);
				  }
			  ...
			  //NOTE: Changing Mi_lower, Mi_lower_actual, etc here is ONLY ok because AGB ejecta is the last thing to be calculated in this Zi loop.
			  Mi_lower = find_initial_mass(t_upper, Zi_correc); //Mass bin (lifetime arrays) corresp. to lowest mass of star to 'die' in current timestep, from SFH bin i.
			  Mi_upper = find_initial_mass(t_lower, Zi_correc); //Mass bin (lifetime arrays) corresp. to highest mass of star to 'die' in current timestep, from SFH bin i.
			  Mi_lower_actual = lifetimeMasses[Mi_lower] + ((lifetimeMasses[Mi_lower+1]-lifetimeMasses[Mi_lower]) * ((t_upper-lifetimes[Zi_correc][Mi_lower])/(lifetimes[Zi_correc][Mi_lower+1]-lifetimes[Zi_correc][Mi_lower]))); //IN MSUN  //Lowest mass of star to 'die' in current timestep from SFH bin i.
			  Mi_upper_actual = lifetimeMasses[Mi_upper] + ((lifetimeMasses[Mi_upper+1]-lifetimeMasses[Mi_upper]) * ((t_lower-lifetimes[Zi_correc][Mi_upper])/(lifetimes[Zi_correc][Mi_upper+1]-lifetimes[Zi_correc][Mi_upper]))); //IN MSUN  //Highest mass of star to 'die' in current timestep from SFH bin i.
