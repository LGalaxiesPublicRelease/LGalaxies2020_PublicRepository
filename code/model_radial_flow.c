...
void gas_inflow(int p, double time)
{
  ...
  if(Gal[p].ColdGas<1.0e-6)
    ...
    {
      rgas=0.5*RingRadius[0]*Gal[p].ColdGasRings[0]; //ROB: The innermost ring is not a 'ring', it's a circle'. Therefore, 0.5*RingRadius[] shouldn't be used to signify the middle of the innermost ring here. 0.75*RingRadius[0] should be used, to give the same ratio as when using 0.5*RingRadius for the outer rings. (28-07-20)
