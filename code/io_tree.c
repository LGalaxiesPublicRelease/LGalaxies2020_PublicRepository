...
void load_tree_table(int filenr)
{
  ...
  //printf("%s/treedata/tree_sf1_dbids_%03d.%d\n", SimulationDir, SnapShotInFileName, filenr);
#endif
   ...
   * get from one first halo to the next. */
  for(i = 1; i < Ntrees; i++)
...
void load_tree(int nr)
{
  ...
  //printf("Allocating to HaloGal: %d\n", MaxHaloGal);
  ...
  //printf("Allocating to Gal: %d, %d\n", sizeof(struct GALAXY) * MaxGal, MaxGal);
  //printf("Allocating to Gal: %d\n", MaxGal);
  ...
  //printf("Allocating to GalTree: %d, %d\n", sizeof(struct galaxy_tree_data) * MaxGalTree, MaxGalTree);
#endif
}
