...
void disruption_code (int galID, double time, int centralgal)
{
  ...
  //terminate("check\n");
	...
	//Rings are in decreasing order. If the disruption radius is between two rings, only transfer a fraction of the material
	int jj;
	for (jj=0;jj<RNUM;jj++) 
		  if (jj != 0 && jj != RNUM - 1 && RingRadius[jj+1] < DisruptRadius && RingRadius[jj] > DisruptRadius){
                    fractionRings[jj]=DisruptRadius/RingRadius[jj];
                  } else if (RingRadius[jj] > DisruptRadius) {
                    fractionRings[jj]=1.;
                  } else {
                    fractionRings[jj]=0.;
                  }
    ...
    transfer_material_with_rings(Gal[galID].CentralGal,"HotGas",galID,"ColdGas",fractionRings,"disrupt.c", __LINE__);
  	...
  	if(InitialDiskMass > 0.0 && DisruptRadius < Gal[galID].DiskRadius)
  	{
		...
		#ifdef H2_AND_RINGS
		//Do nothing as metals have already been transferred
		#else
   ...
   //COLD GAS DISRUPTION
  	if(InitialColdGasMass > 0.0 && DisruptRadius < Gal[galID].ColdGasRadius)
  	{
		...
		#ifdef H2_AND_RINGS
		//Metals have already been transferred
		#else
