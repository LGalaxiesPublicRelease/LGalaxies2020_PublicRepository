 ...
 * 05-05-22: Modified to account for binary_c yields, which need to be integrated over in time for AGBs, SNe-II, and SNe-Ia.
 */
		  ...
		  for (i=0;i<=SFH_ibin[snap][step];i++) {
			  ...
			  for (mb=1;mb<=mbmax;mb++) {
				  ...
				  for (Zi=0;Zi<LIFETIME_Z_NUM;Zi++) {
#ifdef BINARYC
					  ti_lower_actual = t_lower; //Rename the actual lower time [in yr] to something analogous to Mi_lower_actual for other yield sets.
					  ti_upper_actual = t_upper; //Rename the actual higher time [in yr] to something analogous to Mi_upper_actual for other yield sets.
					  ...
					  ti_lower = find_bc_timebin(t_upper*(UnitTime_in_years/Hubble_h)/1.e6); //N.B. the larger lookback time (i.e. t_upper) corresponds to the shorter cosmic time in the binaryc tables (i.e. ti_lower).
					  ti_upper = find_bc_timebin(t_lower*(UnitTime_in_years/Hubble_h)/1.e6);

					  //Find binary_c metallicity bin:
					  Zi_BC = find_initial_metallicity_comp(Zi, i, 5);

					  //Calc yield at lower/upper times:
#ifdef INDIVIDUAL_ELEMENTS
					  calc_ejecta_limits(ti_lower_actual, ti_upper_actual, ti_lower, ti_upper, Zi_BC,
										 BC_TIME_NUM, BC_Z_NUM, bcTimes, bcSNIIEjectedMasses, bcSNIITotalMetals, bcSNIIYields,
					  ...
					  calc_ejecta_limits(ti_lower_actual, ti_upper_actual, ti_lower, ti_upper, Zi_BC,
								 	 	 BC_TIME_NUM, BC_Z_NUM, bcTimes, bcSNIIEjectedMasses, bcSNIITotalMetals,
										 &SNIIEjectedMasses_lower_actual, &SNIIEjectedMasses_upper_actual, &SNIITotalMetals_lower_actual, &SNIITotalMetals_upper_actual);
					  ...
					  for (j=ti_lower;j<=ti_upper;j++) { //loop over binary_c timebins of relevance
...
#ifdef INDIVIDUAL_ELEMENTS
						  calc_integral_limits(j, Zi_BC, ti_lower, ti_upper, ti_lower_actual, ti_upper_actual,
											   BC_TIME_NUM, BC_Z_NUM, bcTimes, bcSNIaEjectedMasses, bcSNIaTotalMetals, bcSNIaYields,
											   SNIaEjectedMasses_lower_actual, SNIaEjectedMasses_upper_actual, SNIaTotalMetals_lower_actual, SNIaTotalMetals_upper_actual, SNIaYields_lower_actual, SNIaYields_upper_actual,
											   &ML, &MU, &EjectedMasses_lower, &EjectedMasses_upper, &TotalMetals_lower, &TotalMetals_upper, &Yields_lower, &Yields_upper);
#else //INDIVIDUAL_ELEMENTS
						  calc_integral_limits(j, Zi_BC, ti_lower, ti_upper, ti_lower_actual, ti_upper_actual,
											   BC_TIME_NUM, BC_Z_NUM, bcTimes, bcSNIaEjectedMasses, bcSNIaTotalMetals,
											   SNIaEjectedMasses_lower_actual, SNIaEjectedMasses_upper_actual, SNIaTotalMetals_lower_actual, SNIaTotalMetals_upper_actual,
											   &ML, &MU, &EjectedMasses_lower, &EjectedMasses_upper, &TotalMetals_lower, &TotalMetals_upper);
#endif //INDIVIDUAL_ELEMENTS

						  //Calc yield ejected in timestep step from stars born in minibin mb of SFH bin i:
						  NormSNIaMassEjecRate[(STEPS*snap)+step][i][Zi] += (MU-ML) * ((EjectedMasses_lower + EjectedMasses_upper)/2.0); //This is a unitless quantity, which must be multiplied by SFR (i.e. M_starsFormedInSFHBin*dt_SFH, in model_yields.c) to get units of [Msun/yr], and then by dt (i.e. timestep width, also in model_yields.c) to get units of [Msun]. (22-05-20)
						  NormSNIaMetalEjecRate[(STEPS*snap)+step][i][Zi] += (MU-ML) * ((TotalMetals_lower + TotalMetals_upper)/2.0); //This is a unitless quantity, which must be multiplied by SFR (in model_yields.c) to get units of [Msun/yr], and then by dt (also in model_yields.c) to get units of [Msun]. (22-05-20) //NO! --> //IN [MSun/yr] //The rate of ejection of 'newly synthesised' metals from a 1 Msun stellar population.
#ifdef INDIVIDUAL_ELEMENTS
						  for (kk=0;kk<NUM_ELEMENTS;kk++) {
#ifndef MAINELEMENTS
							  NormSNIaYieldRate[(STEPS*snap)+step][i][Zi][kk] += NormFactor * (MU-ML) * ((Yields_lower[kk] + Yields_upper[kk])/2.0); //IN [Msun/yr] //The rate of ejection of individual elements from a 1 Msun stellar population.
#else //MAINELEMENTS
							  switch(kk){case 0: kk=0; break; case 1: kk=1; break; case 2: kk=4; break; case 3: kk=6; break; case 4: kk=10; break;}
							  NormSNIaYieldRate[(STEPS*snap)+step][i][Zi][kk] += NormFactor * (MU-ML) * ((Yields_lower[kk] + Yields_upper[kk])/2.0); //IN [Msun/yr] //The rate of ejection of individual elements from a 1 Msun stellar population.
#endif //MAINELEMENTS
...
#ifdef INDIVIDUAL_ELEMENTS
						  calc_integral_limits(j, Zi_BC, ti_lower, ti_upper, ti_lower_actual, ti_upper_actual,
											   BC_TIME_NUM, BC_Z_NUM, bcTimes, bcAGBEjectedMasses, bcAGBTotalMetals, bcAGBYields,
...
#else //INDIVIDUAL_ELEMENTS
						  calc_integral_limits(j, Zi_BC, ti_lower, ti_upper, ti_lower_actual, ti_upper_actual,
											   BC_TIME_NUM, BC_Z_NUM, bcTimes, bcAGBEjectedMasses, bcAGBTotalMetals,
						  ...
						  for (kk=0;kk<NUM_ELEMENTS;kk++) {
#ifndef MAINELEMENTS
							  NormAGBYieldRate[(STEPS*snap)+step][i][Zi][kk] += NormFactor * (MU-ML) * ((Yields_lower[kk] + Yields_upper[kk])/2.0); //IN [Msun/yr] //The rate of ejection of individual elements from a 1 Msun stellar population.
#else //MAINELEMENTS
							  switch(kk){case 0: kk=0; break; case 1: kk=1; break; case 2: kk=4; break; case 3: kk=6; break; case 4: kk=10; break;}
							  NormAGBYieldRate[(STEPS*snap)+step][i][Zi][kk] += NormFactor * (MU-ML) * ((Yields_lower[kk] + Yields_upper[kk])/2.0); //IN [Msun/yr] //The rate of ejection of individual elements from a 1 Msun stellar population.
#endif //MAINELEMENTS
...
#ifdef BINARYC
int find_bc_timebin(double actual_time) //actual_time should be in Myr for this function
{
  int i=-1;
  /*do {
	  i++;
	  bcBinwidth = BC_LOGT_BINWIDTH*bcTimes[i]*log(10.); //Width of binary_c timebin i in (linear) Myr
  }
  while (bcTimes[i]+(0.5*bcBinwidth) < actual_time);
  return i-1; //Returns the binary_c timebin in which LGals_time lies, given that bcTimes[] contains the mid-time in each bin*/
  do {
  	  i++;
  }
...
#endif
{
  ...
  *TotalMetals_lower_actual = TotalMetals[Zi][Mi_lower] + ((TotalMetals[Zi][x_lower+1]-TotalMetals[Zi][x_lower]) * ((x_lower_actual-x_bins[x_lower])/(x_bins[x_lower+1]-x_bins[x_lower])));
#ifdef INDIVIDUAL_ELEMENTS
  for (k=0;k<NUM_ELEMENTS;k++) {
#ifndef MAINELEMENTS
	  kk=k;
	  Yields_lower_actual[k] = Yields[Zi][kk][x_lower] + ((Yields[Zi][kk][x_lower+1]-Yields[Zi][kk][x_lower]) * ((x_lower_actual-x_bins[Mi_lower])/(x_bins[x_lower+1]-x_bins[x_lower])));
#else
  ...
  //Upper limits:
  *EjectedMasses_bins_upper_actual = EjectedMasses_bins[Zi][x_upper] + ((EjectedMasses_bins[Zi][x_upper+1]-EjectedMasses_bins[Zi][x_upper]) * ((x_upper_actual-x_bins[x_upper])/(x_bins[x_upper+1]-x_bins[x_upper])));
