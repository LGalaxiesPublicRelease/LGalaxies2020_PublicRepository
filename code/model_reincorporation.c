  ...
  if (Gal[p].EjectedMass > 0.) {
		for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("IN reincorporate_gas() 1: Gal[%i].EjectedMass_elements[%i] = %e | Gal[%i].HotGas_elements[%i] = %e\n",
						p, ee, Gal[p].EjectedMass_elements[ee], p, ee, Gal[p].HotGas_elements[ee]);
#ifdef DETAILED_DUST
	  //transfer_material(p,"HotGas",p,"EjectedMass",reincorporated/Gal[p].EjectedMass,0.,0.,"model_reincorporation.c", __LINE__);
	  //Change for EjectaDust:
	  transfer_material(p,"HotGas",p,"EjectedMass",reincorporated/Gal[p].EjectedMass,0.,reincorporated/Gal[p].EjectedMass,"model_reincorporation.c", __LINE__);
#else
	  transfer_material(p,"HotGas",p,"EjectedMass",reincorporated/Gal[p].EjectedMass,"model_reincorporation.c", __LINE__);
#endif
	for (int ee=0;ee<NUM_ELEMENTS;ee++)
		if (Gal[p].EjectedMass_elements[ee] < 0.0) {
			printf("IN reincorporate_gas() 2: Gal[%i].EjectedMass_elements[%i] = %e\n", p, ee, Gal[p].EjectedMass_elements[ee]);
			printf("Gal[%i].EjectedMass = %e | reincorporated = %e | Gal[%i].HotGas_elements[%i] = %e\n", p, Gal[p].EjectedMass, reincorporated, p, ee, Gal[p].HotGas_elements[ee]);
		}
  }
