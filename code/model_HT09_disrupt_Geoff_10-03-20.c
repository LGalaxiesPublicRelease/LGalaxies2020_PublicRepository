...
void disruption_code (int galID, double time, int centralgal)
{
  ...
  //Record the initial bulge metals
  double InitialBulgeMetals[NUM_METAL_CHANNELS];

  for (dd=0;dd<NUM_METAL_CHANNELS;dd++) {
	InitialBulgeMetals[dd] = Gal[galID].MetalsBulgeMass[dd];
  }
  ...
  if(DisruptRadius < 1e-6)
 	  DisruptRadius = 0.0;
	//printf("%0.3e %0.3e\n",DisruptRadius,max(max(Gal[galID].ColdGasRadius,Gal[galID].DiskRadius),Gal[galID].BulgeSize));



  if(DisruptRadius<max(max(Gal[galID].ColdGasRadius,Gal[galID].DiskRadius),Gal[galID].BulgeSize))
  {
  //terminate("check\n");
  	//disk disruption
	...
	//InitialICM = Gal[Gal[galID].CentralGal].ICM;

	//M_PI*RingRadius[i]*RingRadius[i];
	//RingArea[i] = M_PI*(RingRadius[i]*RingRadius[i]-RingRadius[i-1]*RingRadius[i-1]);

	if (RingRadius[0] > DisruptRadius) {
	    if (DisruptRadius == 0.) {
		fractionRings[0] = 1.;
	    } else {
		fractionRings[0] = (RingArea[0] - M_PI*DisruptRadius*DisruptRadius)/RingArea[0];
	    }
	}
	for (jj=1;jj<RNUM;jj++) {
		  if (RingRadius[jj-1] < DisruptRadius && RingRadius[jj] > DisruptRadius){
                    fractionRings[jj] = (RingArea[jj] - M_PI*(DisruptRadius*DisruptRadius-RingRadius[jj-1]*RingRadius[jj-1]))/RingArea[jj];
                  } else if (RingRadius[jj-1] < DisruptRadius && RingRadius[jj] < DisruptRadius) {
                    fractionRings[jj]=1.;
                  } else {
                    fractionRings[jj]=0.;
                  }
        ...
        transfer_material_with_rings(Gal[galID].CentralGal,"ICM",galID,"DiskMass",fractionRings,"disrupt.c", __LINE__);

        transfer_material_with_rings(Gal[galID].CentralGal,"HotGas",galID,"ColdGas",fractionRings,"disrupt.c", __LINE__);

	#endif

  	if(InitialDiskMass > 0.0 && DisruptRadius < Gal[galID].DiskRadius)
  	{
  		Sigma0_disk = Gal[galID].DiskMass / (2 * M_PI * Gal[galID].DiskRadius / 3.0 * Gal[galID].DiskRadius / 3.0);
		#ifndef H2_AND_RINGS
  		Gal[galID].DiskMass = diskmass(DisruptRadius/(Gal[galID].DiskRadius / 3.0));
  		#endif
		Gal[galID].DiskRadius = DisruptRadius;

  		if(Gal[galID].DiskMass > InitialDiskMass)
  			Gal[galID].DiskMass=InitialDiskMass;

  		if(Gal[galID].DiskMass < 1e-6)
  			Gal[galID].DiskMass=Gal[galID].DiskRadius=0.0;
 
		#ifndef H2_AND_RINGS
		//Gal[galID].MetalsDiskMass*=Gal[galID].DiskMass/InitialDiskMass;
		for (hh=0;hh<NUM_METAL_CHANNELS;hh++) {
		Gal[galID].MetalsDiskMass[hh]*=Gal[galID].DiskMass/InitialDiskMass;
		}
		#endif
	}
  	...
  	{
		Gal[galID].BulgeMass= bulgemass(DisruptRadius/Gal[galID].BulgeSize);
		...
		Gal[galID].MetalsBulgeMass[mm]*=Gal[galID].BulgeMass/InitialBulgeMass;
		}
  	}
	...
	if(Gal[galID].BulgeMass < InitialBulgeMass)
  	{
		...
		//Edit: The bulge does not have rings, so transfer the mass, metals, and elements here
		Gal[Gal[galID].CentralGal].ICM+=(InitialBulgeMass-Gal[galID].BulgeMass);
		
		for (yy=0;yy<NUM_ELEMENTS;yy++) {
			Gal[Gal[galID].CentralGal].ICM_elements[yy]+=Gal[galID].BulgeMass_elements[yy]*(1-(Gal[galID].BulgeMass/InitialBulgeMass));
			Gal[galID].BulgeMass_elements[yy]*=Gal[galID].BulgeMass/InitialBulgeMass;
		}

		for (cc=0;cc<NUM_METAL_CHANNELS;cc++) {
			Gal[Gal[galID].CentralGal].MetalsICM[cc]+=InitialBulgeMetals[cc]-Gal[galID].MetalsBulgeMass[cc];
		}
   ...
   //COLD GAS DISRUPTION
  	if(InitialColdGasMass > 0.0 && DisruptRadius < Gal[galID].ColdGasRadius)
  	{
  		Sigma0_disk = Gal[galID].ColdGas / (2 * M_PI * Gal[galID].ColdGasRadius / 3.0 * Gal[galID].ColdGasRadius / 3.0);
  		
		#ifndef H2_AND_RINGS
                //For H2_AND_RINGS, the mass has already been transferred
		Gal[galID].ColdGas = diskmass(DisruptRadius/(Gal[galID].ColdGasRadius / 3.0));
		#endif
  		
		Gal[galID].ColdGasRadius = DisruptRadius;

  		if(Gal[galID].ColdGas > InitialColdGasMass)
  			Gal[galID].ColdGas=InitialDiskMass;

  		if(Gal[galID].ColdGas < 1e-6)
  			Gal[galID].ColdGas=Gal[galID].ColdGasRadius=0.0;
		...
		for (aa=0;aa<NUM_METAL_CHANNELS;aa++) {
		  //Gal[Gal[galID].CentralGal].MetalsHotGas+=Gal[galID].MetalsColdGas*(1.-Gal[galID].ColdGas/InitialColdGasMass);
		  Gal[Gal[galID].CentralGal].MetalsHotGas[aa]+=Gal[galID].MetalsColdGas[aa]*(1.-Gal[galID].ColdGas/InitialColdGasMass);
  		  //Gal[galID].MetalsColdGas *= Gal[galID].ColdGas/InitialColdGasMass;
		  Gal[galID].MetalsColdGas[aa]*=Gal[galID].ColdGas/InitialColdGasMass;
		}
		#endif
...
double disruption_radius(int p, int centralgal)
{
  ...
  Rgasdisk = Gal[p].ColdGasRadius/3.;
  Rdisk = Gal[p].DiskRadius/3.;
  ...
  do {
    ...
    if(Mbulge > 0.0)
    	SatelliteMass_R += bulgemass(DisruptRadius/Rbulge);
    SatelliteMass_R += diskmass(DisruptRadius/Rdisk);
    SatelliteMass_R += diskmass(DisruptRadius/Rgasdisk);
