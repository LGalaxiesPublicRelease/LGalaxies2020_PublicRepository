...
void grow_black_hole(int merger_centralgal, double mass_ratio, double deltaT)
{
  ...
  if(Gal[merger_centralgal].ColdGas > 0.0)
	...
	{
#ifdef DETAILED_DUST
    	  transfer_material_with_rings(merger_centralgal,"BlackHoleMass",merger_centralgal,"ColdGas", fractionRings, fractionRings, fractionRings, "model_mergers.c", __LINE__);
#else
...
#ifdef DETAILED_DUST
    	  transfer_material_with_rings(merger_centralgal,"BlackHoleGas",merger_centralgal,"ColdGas", fractionRings, fractionRings, fractionRings, "model_mergers.c", __LINE__);
#else
      ...
      if (BlackHoleGrowth == 0)
      	{
#ifdef DETAILED_DUST
    	  transfer_material(merger_centralgal,"BlackHoleMass",merger_centralgal,"ColdGas",fraction,fraction,fraction,"model_mergers.c", __LINE__);
#else
...
#ifdef DETAILED_DUST
    	  transfer_material(merger_centralgal,"BlackHoleGas",merger_centralgal,"ColdGas",fraction,fraction,fraction,"model_mergers.c", __LINE__);
#else
...
void add_galaxies_together(int t, int p, double deltaT)
{
  ...
  //ROB: This seems to be re-arranging the gas in the satellite galaxy to follow the exact radial profile defined above: (01-02-22):
  for(j=0; j<RNUM; j++)
    {
...
#endif
#endif
      fractionRings[j]=1.; //fractionRings is made = 1. here for every ring, ready for use by transfer_material_with_rings() below. I.e. all gas in each ring should be transferred.
    }
#ifdef DETAILED_DUST
  transfer_material_with_rings(t,"ColdGas",p,"ColdGas",fractionRings,fractionRings,fractionRings,"model_mergers.c", __LINE__);
  //N.B. When dust is resolved into rings too, we'll need to pass fractionRings for the cloud and diffuse fractions here, rather than just a 1. float.
#else
  transfer_material_with_rings(t,"ColdGas",p,"ColdGas",fractionRings,"model_mergers.c", __LINE__);
#endif //DETAILED_DUST
#else //H2_AND_RINGS
#ifdef DETAILED_DUST
  transfer_material(t,"ColdGas",p,"ColdGas",1.,1.,1.,"model_mergers.c", __LINE__);
...
#ifdef DETAILED_DUST
  transfer_material(t,"HotGas",p,"HotGas",1.,0.,1.,"model_mergers.c", __LINE__); //When DUST_HOTGAS is on, Diff_fraction is set to 1.0 so that dust in the HotGas of p are 100% transferred to t.
  //transfer_material(t,"EjectedMass",p,"EjectedMass",1.,0.,0.,"model_mergers.c", __LINE__);
  //Change for EjectaDust:
  transfer_material(t,"EjectedMass",p,"EjectedMass",1.,0.,1.,"model_mergers.c", __LINE__); //Diff_fraction is set to 1.0 so that, when DUST_EJECTEDMASS is on, dust in the EjecedMass of p is 100% transferred to t.
#else
...
#ifdef DETAILED_DUST
  transfer_material(t,"BurstMass",p,"BurstMass",1.,0.,0.,"model_mergers.c", __LINE__);
#else
  ...
  if(BulgeFormationInMinorMergersOn)
    transfer_material_with_rings(t,"BulgeMass",p,"DiskMass",fractionRings,fractionCloudsRings,fractionDiffRings,"model_mergers.c", __LINE__); //ROB: This transfers 100% of the sat's disc into the central's bulge for minor mergers. (08-11-21)
  else
    transfer_material_with_rings(t,"DiskMass",p,"DiskMass",fractionRings,fractionCloudsRings,fractionDiffRings,"model_mergers.c", __LINE__);

  if(Gal[p].BulgeMass>0.)
    transfer_material_with_rings(t,"BulgeMass",p,"BulgeMass",fractionRings,fractionCloudsRings,fractionDiffRings,"model_mergers.c", __LINE__);
#else
  if(BulgeFormationInMinorMergersOn)
...
#ifdef DETAILED_DUST
  if(BulgeFormationInMinorMergersOn)
    transfer_material(t,"BulgeMass",p,"DiskMass",1.,0.,0.,"model_mergers.c", __LINE__);
  else
    transfer_material(t,"DiskMass",p,"DiskMass",1.,0.,0.,"model_mergers.c", __LINE__);
  if(Gal[p].BulgeMass>0.)
    transfer_material(t,"BulgeMass",p,"BulgeMass",1.,0.,0.,"model_mergers.c", __LINE__);
  transfer_material(t,"ICM",p,"ICM",1.,0.,0.,"model_mergers.c", __LINE__);
#else
  if(BulgeFormationInMinorMergersOn)
...
void make_bulge_from_burst(int p)
{
  ...
  for (jj=0;jj<RNUM;jj++) {
	  fractionRings[jj]=1.;
#ifdef DETAILED_DUST
	  fractionCloudsRings[jj]=0.;
	  fractionDiffRings[jj]=0.;
#endif
  }

#ifdef DETAILED_DUST
  transfer_material_with_rings(p,"BulgeMass",p,"DiskMass",fractionRings,fractionCloudsRings,fractionDiffRings,"model_mergers.c", __LINE__);
#else
  transfer_material_with_rings(p,"BulgeMass",p,"DiskMass",fractionRings,"model_mergers.c", __LINE__);
#endif //DETAILED_DUST
#else //H2_AND_RINGS
#ifdef DETAILED_DUST
  transfer_material(p,"BulgeMass",p,"DiskMass",1.,0.,0.,"model_mergers.c", __LINE__);
#else
  ...
  /* ROB: (05-06-23): N.B. The code below means that SUM(SfrRings) != Sfr in the outputs,
  ** as the rings values are set to 0.0 but the Sfr isn't (i.e. it still includes newly-formed stars
  ** that were transfered to the bulge component).
  */
  Gal[p].SfrBulge  = Gal[p].Sfr;
#ifdef H2_AND_RINGS
  for(jj=0;jj<RNUM;jj++)
	  {
	  Gal[p].SfrRings[jj]=0.0;
	  Gal[p].SfrInstRings[jj]=0.0; //*****ROB*****//
	  }
#endif
				  ...
				  double time, double deltaT)
{
  ...
  //ROB: (05-06-23) Added these checks to make sure that ColdGas isn't nan, inf, or negative before assigning its value to mstars:
  if(mstars > Gal[merger_centralgal].ColdGas) {
	  if (isnormal(Gal[merger_centralgal].ColdGas) == 0 || Gal[merger_centralgal].ColdGas < 0.0)
		  mstars = 0.0;
	  else
  ...
  for(j=0;j<RNUM;j++) {
    if(mstarsRings[j] > Gal[merger_centralgal].ColdGasRings[j])
    	if (isnormal(Gal[merger_centralgal].ColdGasRings[j]) == 0 || Gal[merger_centralgal].ColdGasRings[j] < 0.0)
    		mstarsRings[j] = 0.0;
    	else
...
//there is no need to balance it with SF
  ...
  for(j=0;j<RNUM;j++) {
    ...
    SfrRingsTot += mstarsRings[j] / deltaT;
  }
#endif
  mass_checks(merger_centralgal,"model_mergers.c",__LINE__);

  if (((mstars/deltaT) / SfrRingsTot < 0.99999) || ((mstars/deltaT) / SfrRingsTot > 1.00001))
 	 printf("\nmodel_mergers.c: Sfr_added = %e | SfrRingsTot_added = %e\n", mstars/deltaT, SfrRingsTot);
  ...
  if (mstars > 0.)
#ifndef H2_AND_RINGS
    SN_feedback(merger_centralgal, centralgal, mstars, "ColdGas", deltaT/STEPS);
#else
    SN_feedback(merger_centralgal, centralgal, mstars, mstarsRings, "ColdGas", deltaT/STEPS);
#endif
#endif
			   ...
			   double RgasCentral, double RStellarDiskCentral, double RgasSat, double RStellarDiskSat)
{
  ...
  double Msat,Rsat;
  double fint=0.5;
  double c=0.5; //ROB: Not sure declaring/defining a local variable called "c" is a good idea... (08-11-21)
...
#endif*/
//#####
//ROB: I added the redistribution of elements in the bulge rings here too.
//(I think not doing this was causing SUM(sfh_BulgeMass_elementsRings) = 0.0 even though sfh_BulgeMassRings > 0.0) (06-07-22)
#ifdef STAR_FORMATION_HISTORY
      for (ii=0; ii<=Gal[merger_centralgal].sfh_ibin; ii++) {
    	  Gal[merger_centralgal].sfh_BulgeMassRings[jj][ii] = fractionRings[jj]*Gal[merger_centralgal].sfh_BulgeMass[ii];
#ifdef INDIVIDUAL_ELEMENTS
    	  for(kk=0;kk<NUM_ELEMENTS;kk++)
    		  Gal[merger_centralgal].sfh_BulgeMass_elementsRings[jj][ii][kk] = fractionRings[jj]*Gal[merger_centralgal].sfh_BulgeMass_elements[ii][kk];
#endif //INDIVIDUAL_ELEMENTS
      }
