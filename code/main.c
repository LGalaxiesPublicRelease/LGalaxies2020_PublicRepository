...
//***************************************************************************************
  ...
  //for(treenr = 0; treenr <100;treenr++)
  {
    ...
    //printf("doing tree %d of %d\n", treenr, Ntrees);
      ...
      //printf("output_galaxy done\n");

#ifndef MCMC
#ifdef GALAXYTREE
      save_galaxy_tree_finalize(filenr, treenr);
      //printf("save_galaxy_tree_finalize done\n");
#ifndef PARALLEL
...
int join_galaxies_of_progenitors(int halonr, int ngalstart, int *FOF_centralgal)
{
	...
	for(i = 0, currentgal = HaloAux[prog].FirstGalaxy; i < HaloAux[prog].NGalaxies; i++) {
	    ...
	    Gal[ngal].DiskSNIaRate = 0.0;
	    Gal[ngal].BulgeSNIaRate = 0.0;
	    Gal[ngal].ICMSNIaRate = 0.0;
	    ...
	    Gal[ngal].Sfr = 0.0;
	    ////Gal[ngal].NumDisr = 0; // Don't reset to zero here for cumulative variables like NumDisr //***** ROB: Geoff's disruption counter (24-03-20) *****
#ifdef H2_AND_RINGS
	  for(j=0;j<RNUM;j++)
	  {
	  ...
	  Gal[ngal].DustColdGasRates[0] = 0.0;
	  Gal[ngal].DustColdGasRates[1] = 0.0;
	  Gal[ngal].DustColdGasRates[2] = 0.0;
	  Gal[ngal].DustColdGasRates[3] = 0.0;
	  Gal[ngal].DustColdGasRates[4] = 0.0;
#ifdef DUST_HOTGAS
	  //Gal[ngal].DustHotGasRates = 0.0;
	  Gal[ngal].DustHotGasRates[0] = 0.0;
	  Gal[ngal].DustHotGasRates[1] = 0.0;
	  Gal[ngal].DustHotGasRates[2] = 0.0;
	  Gal[ngal].DustHotGasRates[3] = 0.0;
#endif //DUST_HOTGAS
#ifdef DUST_EJECTEDMASS
	  Gal[ngal].DustEjectedMassRates = 0.0;
#endif //DUST_EJECTEDMASS
#endif //FULL_DUST_RATES
	  for (int l=0; l<NUM_ELEMENTS; l++) {
#ifdef H2_AND_RINGS
		  for(j=0;j<RNUM;j++) {
			  Gal[ngal].f_i[j][l] = 0.0;
			  Gal[ngal].f_c[j][l] = 0.0;
		  }
#else //H2_AND_RINGS
		  Gal[ngal].f_i[l] = 0.0;
		  Gal[ngal].f_c[l] = 0.0;
      ...
      //Gal[ngal].t_des = 0.0; //ROB: Not needed after all (I think), as Gal[p].t_des is overwritten in each timestep in model_dust_yields.c anyway (10-02-22)
#ifdef TAU_RINGS
	  for(j=0;j<RNUM;j++)
		  Gal[ngal].t_acc[j] = 0.0;
#else //TAU_RINGS
  	  Gal[ngal].t_acc = 0.0;
#endif //TAU_RINGS
#endif //DETAILED_DUST
...
void evolve_galaxies(int halonr, int ngal, int treenr)
...
#endif
    double tot_ele_clouds=0.0, tot_ele_diff=0.0, met_ele_clouds=0.0, met_ele_diff=0.0;
    double tot_ele_clouds_rings[RNUM], tot_ele_diff_rings[RNUM], met_ele_clouds_rings[RNUM], met_ele_diff_rings[RNUM];
   ...
   * Needed in case some galaxy has skipped a snapshot. */
    ...
    for (p=0; p<ngal; p++) {
    	...
    	/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("AFTER first sfh_update_bins(): Gal[%i].EjectedMass_elements[%i] = %e | Gal[%i].HotGas_elements[%i] = %e\n",
						p, ee, Gal[p].EjectedMass_elements[ee], p, ee, Gal[p].HotGas_elements[ee]);*/
    }
#endif
	...
	for (p=0; p<ngal; p++) {
		...
		/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("AFTER second sfh_update_bins(): %i: Gal[%i].EjectedMass_elements[%i] = %e | Gal[%i].HotGas_elements[%i] = %e\n",
						nstep, p, ee, Gal[p].EjectedMass_elements[ee], p, ee, Gal[p].HotGas_elements[ee]);*/
	}
#endif
	...
	for (p = 0; p < ngal; p++) {
		...
		*/
		...
		/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("BEFORE reincorporate_gas(): %i: Gal[%i].EjectedMass_elements[%i] = %e | Gal[%i].HotGas_elements[%i] = %e | infallingGas = %e\n",
						nstep, p, ee, Gal[p].EjectedMass_elements[ee], p, ee, Gal[p].HotGas_elements[ee], infallingGas);*/
	...
	for (p = 0; p < ngal; p++) {
		/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("AFTER do_AGN_heating(): %i: Gal[%i].EjectedMass_elements[%i] = %e\n", nstep, p, ee, Gal[p].EjectedMass_elements[ee]);*/

		cool_gas_onto_galaxy(p, deltaT / STEPS);
	    mass_checks(p,"main.c",__LINE__);
		/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("AFTER cool_Gas-Onto_galaxy(): %i: Gal[%i].EjectedMass_elements[%i] = %e\n", nstep, p, ee, Gal[p].EjectedMass_elements[ee]);*/
	...
	for(p = 0; p < ngal; p++) {
		/*for (int ee=0;ee<NUM_ELEMENTS;ee++)
			if (Gal[p].EjectedMass_elements[ee] < 0.0)
				printf("BEFORE mergers: %i: Gal[%i].EjectedMass_elements[%i] = %e\n", nstep, p, ee, Gal[p].EjectedMass_elements[ee]);*/
...
//ROB: I have added another update of the H2 fraction here, as I think this can change during mergers and BH growth, which are run just above. (09-03-22)
#ifdef H2_AND_RINGS
	for (p = 0; p < ngal; p++) {
		update_h2fraction(p);
	}
...
#ifdef DETAILED_DUST
		update_dust_mass(p, FOF_centralgal, deltaT/STEPS, nstep, halonr);

		mass_checks(p,"update_dust_mass #0",__LINE__);
		mass_checks(FOF_centralgal,"update_dust_mass #1",__LINE__);
...
//This takes place inside update_yields_and_return_mass() if DETAILED_DUST is OFF:
    	if(TotalMassReturnedToColdDiskGas>0.) {
    		if(TotalMassReturnedToColdDiskGas>Gal[p].ColdGas)
    			TotalMassReturnedToColdDiskGas=Gal[p].ColdGas; //ROB: Shouldn't we do the same check for TotalMassReturnedToColdDiskGasr here too? (29-11-21)
#ifndef H2_AND_RINGS
    		SN_feedback(p, FOF_centralgal, TotalMassReturnedToColdDiskGas, "ColdGas", deltaT/STEPS);
#else //H2_AND_RINGS
    		SN_feedback(p, FOF_centralgal, TotalMassReturnedToColdDiskGas, TotalMassReturnedToColdDiskGasr, "ColdGas", deltaT/STEPS);
#endif //H2_AND_RINGS
    		Gal[p].MassReturnRateToColdGas += TotalMassReturnedToColdDiskGas / deltaT; //ROB: Store mass return rate by SNe and stellar winds to the gas phases.
    	}
...
#ifndef H2_AND_RINGS
    		SN_feedback(p, FOF_centralgal, TotalMassReturnedToHotGas, "HotGas", deltaT/STEPS);
#else //H2_AND_RINGS
    		double HotGasRings[RNUM];
    		int jj;
    		for(jj=0;jj<RNUM;jj++)
    			HotGasRings[jj]=0.;
    		SN_feedback(p, FOF_centralgal, TotalMassReturnedToHotGas, HotGasRings, "HotGas", deltaT/STEPS);
#endif //H2_AND_RINGS
    		Gal[p].MassReturnRateToHotGas += TotalMassReturnedToHotGas / deltaT; //ROB: Store mass return rate by SNe and stellar winds to the gas phases (from disk stars, bulge stars, and ICL stars).
    	}

    	if(TotalMetalsReturnedToHotGas>0.) //NOTE: Only metals returned to the HotGas from disc stars & SNe are considered in TotalMetalsReturnedToHotGas (i.e. not from the bulge or ICL):
    		Gal[p].MetalsReturnRateToHotGas += TotalMetalsReturnedToHotGas / deltaT; //ROB: Store mass return rate by SNe and stellar winds to the gas phases (from disk stars, bulge stars, and ICL stars).

    	//if (Gal[p].SnapNum > 50) printf("main.c: SnapNum = %i | DustColdGasRates[3] = %e\n", Gal[p].SnapNum, Gal[p].DustColdGasRates[3]);
     ...
     * disrupted since usually baryonic component is more compact than dark
     * matter.*/
    for(p = 0; p < ngal; p++) {
	if(Gal[p].Type == 2) {
