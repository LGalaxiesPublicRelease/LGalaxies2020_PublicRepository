...
double calc_beta_function_integral(double Rmax, double rc, double beta, char form[]) {
	...
	else if (strcmp(form,"cubed")==0)
		beta = 3.*beta; //If the integral of r^2 * betaFunction^3 is required
	//else the integral of r^2 * betaFunction is done
	//integral = (Rmax/2.) * pow2(Rmax) * pow((1.+pow2(Rmax/rc)), -(3.*beta/2.)); //[distance^3]
	//integral = (pow3(Rmax)/2.) * pow((1.+pow2(Rmax/rc)), -(3.*beta/2.)); //[distance^3]
...
double calc_sputtering(int p, double TotDustMass, double dt, char comp[]) {
	...
	*/
	...
	if (strcmp(comp,"HotGas")==0) {
		//GasDensity = IsothermalGasDensity; //(CompMass*UnitMass_in_g) / ((4./3.) * PI * pow3(Gal[p].Rvir*UnitLength_in_cm/Hubble_h)); //[g/cm^3]
		GasDensity = (CompMass*UnitMass_in_g/Hubble_h) / (4. * PI * calc_beta_function_integral((Gal[p].Rvir*UnitLength_in_cm/Hubble_h), (core_radius*UnitLength_in_cm/Hubble_h), beta, "single")); //[g/cm^3] //This is the core density
	}
	else if (strcmp(comp,"EjectedMass")==0) {
		//GasDensity = IsothermalGasDensity; //(CompMass*UnitMass_in_g) / ((4./3.) * PI * pow3(Gal[p].Rvir*UnitLength_in_cm/Hubble_h)); //[g/cm^3] //For now, I am assuming the isothermal density within the halo for the EjectedMass gas density too... (01-03-23)
		GasDensity = (CompMass*UnitMass_in_g/Hubble_h) / (4. * PI * calc_beta_function_integral((Gal[p].Rvir*UnitLength_in_cm/Hubble_h), (core_radius*UnitLength_in_cm/Hubble_h), beta, "single")); //[g/cm^3] //This is the core density
	}
	...
	//ROB: (16-03-22): Could have a galaxy struct variable which stores current dust grain radius, for use here like this:
	//Gal[p].DustHotGas_grainRadius -= Delta_grainRadius;
	//tau_sput = sputCharacTime * (Gal[p].DustHotGas_grainRadius/CharacGrainRadius) * (CharacDensity/GasDensity) * (pow(sputT0/Tvir,sputOmega)+1.); //[Gyr] //Eqn.23 Triani+20

	//*****
	//Calculate tau_sput:
	//a) Assuming this depends on GasDensity calculated above (i.e. either the isothermal density or core density):
	//tau_sput = sputCharacTime * ((CharacGrainRadius-Delta_grainRadius)/CharacGrainRadius) * (CharacDensity/GasDensity) * (pow(sputT0/Tvir,sputOmega)+1.); //[Gyr] //Eqn.23 Triani+20, using the core (or isothermal) density
	//b) Assuming the average gas density from the beta profile:
	if (strcmp(comp,"HotGas")==0) {
...
/*
		...
		//NOTE: Need to check Hubble_h factors for all of these constants! (See h_params.h)
		double Tvir, HotGasDensity, Delta_grainRadius, tau_sput;
		HotGasDust = 0.0; //[Msun]
		for (ee=0; ee<NUM_ELEMENTS; ee++)
			HotGasDust += Gal[p].DustHotGas_elements[ee];
