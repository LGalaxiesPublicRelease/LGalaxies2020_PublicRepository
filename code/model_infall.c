...
double infall_recipe(int ngal)
{
    ...
    /*  need to add up all the baryonic mass associated with the full halo to check
	...
	if ( dis < rvir ) {
	    ...
	    tot_mass += Gal[i].ColdGas + Gal[i].HotGas  + Gal[i].EjectedMass + Gal[i].BlackHoleGas; //ROB: N.B. BlackHoleGas should be 0.0 at all times if BlackHoleGrowth != 1. (08-11-21).
#ifndef INFALL_UPDATE
	}
#endif
    }
    ...
    /*if (infallingMass < 0.0)
    	printf("IN infall_recipe() 2: infallingMass = %e\n", infallingMass);*/
...
//if infallingGas is negative set the limit to be removed at hotgas
...
#ifdef DETAILED_DUST
	transfer_material(FOF_centralgal,"ExcessMass",FOF_centralgal,"HotGas",fraction,0.,0.,"add_infall_to_hot", __LINE__);
#else
...
#else
	Gal[FOF_centralgal].HotGas += infallingGas; //standard model i.e. with EXCESS_MASS off
#endif

#ifdef INDIVIDUAL_ELEMENTS
    Gal[FOF_centralgal].HotGas_elements[0] += PRISTINE_H_FRACTION*(infallingGas/Hubble_h)*1.0e10; //H  //standard model i.e. with EXCESS_MASS off
    Gal[FOF_centralgal].HotGas_elements[1] += PRISTINE_HE_FRACTION*(infallingGas/Hubble_h)*1.0e10; //He  //standard model i.e. with EXCESS_MASS off
    //printf("IN add_infall_to_hot(): infallingGas = %e | Gal[%i].HotGas_elements[0] = %e\n", infallingGas, FOF_centralgal, Gal[FOF_centralgal].HotGas_elements[0]);
#endif
	...
	if (infallingGas > TINY_MASS) {
	    ...
	    if (newgas > 0.) {
		Gal[FOF_centralgal].ExcessMass_elements[0] += PRISTINE_H_FRACTION*newgas*1.0e10/Hubble_h; //H
		Gal[FOF_centralgal].ExcessMass_elements[1] += PRISTINE_HE_FRACTION*newgas*1.0e10/Hubble_h; //He
	    }
#endif
...
#ifdef DETAILED_DUST
	    	transfer_material(FOF_centralgal,"HotGas",FOF_centralgal,"EjectedMass",fraction,0.,0.,"add_infall_to_hot", __LINE__);
#else
...
#ifdef DETAILED_DUST
	    transfer_material(FOF_centralgal,"EjectedMass",FOF_centralgal,"ExcessMass",fraction,0.,0.,"add_infall_to_hot", __LINE__);
#else
	    ...
	    if (outflowingGas > TINY_MASS) {
...
#ifdef DETAILED_DUST
				transfer_material(FOF_centralgal,"ExcessMass",FOF_centralgal,"EjectedMass",M_outfromEjected/Gal[FOF_centralgal].EjectedMass,0.,0.,"add_infall_to_hot", __LINE__);
#else
...
#ifdef DETAILED_DUST
				transfer_material(FOF_centralgal,"EjectedMass",FOF_centralgal,"HotGas",fraction,0.,0.,"add_infall_to_hot", __LINE__);
#else
			...
			if (outflowingGas > TINY_MASS) {
#ifdef DETAILED_DUST
				transfer_material(FOF_centralgal,"ExcessMass",FOF_centralgal,"EjectedMass",outflowingGas/Gal[FOF_centralgal].EjectedMass,0.,0.,"add_infall_to_hot", __LINE__);
#else
