...
/* ROB: I have now replaced this method with a new one, below (27-05-22):
//If mass return or unprocessed metals larger than what is currently in the stellar population, re-adjust
    	...
    	double SNII_scale_fraction = 1.0, SNIa_scale_fraction = 1.0, AGB_scale_fraction = 1.0;
#ifdef H2_AND_RINGS
    	...
    	if(SNIIUnProcessedMetals>SNIImetals) {
    		SNII_scale_fraction = SNIImetals/SNIIUnProcessedMetals;
    		SNIIUnProcessedMetals=SNIImetals;
    	}
    	if(SNIaUnProcessedMetals>SNIametals) {
    		SNIa_scale_fraction = SNIametals/SNIaUnProcessedMetals;
    		SNIaUnProcessedMetals=SNIametals;
    	}
    	if(AGBUnProcessedMetals>AGBmetals) {
    		//printf("\n\nWARNING: AGBUnProcessedMetals set to AGBmetals:\n");
    		//printf("AGBUnProcessedMetals = %e Msun | AGBmetals = %e Msun\n", AGBUnProcessedMetals*(1.e10/Hubble_h), AGBmetals*(1.e10/Hubble_h));
    		AGB_scale_fraction = AGBmetals/AGBUnProcessedMetals; //NOTE: For those cases where AGBmetals=0.0, then AGB_scale_fraction=0.0 here, as required.
    		AGBUnProcessedMetals=AGBmetals;
    	}
//ROB: This bit scales the unprocessed METAL elements in the same way as is done for the metals components above: (27-05-22):
#ifdef INDIVIDUAL_ELEMENTS
    	for(ee=2;ee<NUM_ELEMENTS;ee++) { //NOTE: We are counting from ee=2 (i.e. from AFTER helium) here
    		SNIIUnProcessedElements[ee] *= SNII_scale_fraction;
    		SNIaUnProcessedElements[ee] *= SNIa_scale_fraction;
    		AGBUnProcessedElements[ee] *= AGB_scale_fraction;
    	}
#endif //INDIVIDUAL_ELEMENTS
*/

//New way to scale unprocessed components: Ensure that the total metals/each element removed doesn't exceed the total metals/each element currently in the stars:
    	//Metals:
		...
		if (UnprocessedMetals > Metals) {
			//MetalDiff = UnprocessedMetals - Metals;
			...
			//printf("Disc: UnprocessedMetalsScaleFactor = %f | SNIIUnProcessedMetals = %e\n", UnprocessedMetalsScaleFactor, SNIIUnProcessedMetals);
    	...
    	for(ee=0;ee<NUM_ELEMENTS;ee++) {
    		...
    		if (UnProcessedElements > Element) {
    			...
    			//printf("Disc: UnprocessedElementsScaleFactor = %f\n", UnprocessedElementsScaleFactor);
    	...
    	/*for(ee=0;ee<NUM_ELEMENTS;ee++) {
    		if (ee <=1) {
    			//SNIIUnProcessedElements[ee] /= UnprocessedMetalsScaleFactor;
    			//SNIaUnProcessedElements[ee] /= UnprocessedMetalsScaleFactor;
    			//AGBUnProcessedElements[ee] /= UnprocessedMetalsScaleFactor;
    			//Add the missing material to the H and He unprocessed components equally (across all three channels, each):
    			SNIIUnProcessedElements[ee] += (1./3.)*0.5*MetalDiff*(1.e10/Hubble_h);
				SNIaUnProcessedElements[ee] += (1./3.)*0.5*MetalDiff*(1.e10/Hubble_h);
				AGBUnProcessedElements[ee] += (1./3.)*0.5*MetalDiff*(1.e10/Hubble_h);
    		}
    		else {
				SNIIUnProcessedElements[ee] *= (1./3.)*UnprocessedMetalsScaleFactor;
				SNIaUnProcessedElements[ee] *= (1./3.)*UnprocessedMetalsScaleFactor;
				AGBUnProcessedElements[ee] *= (1./3.)*UnprocessedMetalsScaleFactor;
    		}
    	}*/
#endif //INDIVIDUAL_ELEMENTS
    	...
    	//Total:
     	 ...
     	 */
     	/*printf("\nMass into stars = %e | Mass ejected into gas = %e\n", Gal[p].sfh_DiskMassRings[jj][ii], SNIIEjectaMass+SNIaEjectaMass+AGBEjectaMass);
     	printf("Metal mass into stars = %e | Metal mass removed from stars = %e | Metal mass ejected into gas = %e\n",
     			Gal[p].sfh_DiskMassRings[jj][ii]*DiskMetallicity, SNIIUnProcessedMetals+SNIaUnProcessedMetals+AGBUnProcessedMetals, SNIIAllMetals+SNIaAllMetals+AGBAllMetals);
     	printf("Formation Z = %e | Remnant Z = %e | Ejecta Z = %e\n",
     			DiskMetallicity, ((SNIIUnProcessedMetals+SNIaUnProcessedMetals+AGBUnProcessedMetals)-(Gal[p].sfh_DiskMassRings[jj][ii]*DiskMetallicity))/((SNIIEjectaMass+SNIaEjectaMass+AGBEjectaMass)-Gal[p].sfh_DiskMassRings[jj][ii]), (SNIIAllMetals+SNIaAllMetals+AGBAllMetals)/(SNIIEjectaMass+SNIaEjectaMass+AGBEjectaMass));*/
	    ...
	    //##########
	    //Check ejected and removed masses are consistent:
     	double Sum_ele_ej = 0.0, Sum_met_ele_ej = 0.0, Sum_ele_rem = 0.0, Sum_met_ele_rem = 0.0, Sum_ele = 0.0, Sum_met_ele = 0.0;
     	double met_ratio = 0.0, met_rem_ratio = 0.0;
		for(ee=0;ee<NUM_ELEMENTS;ee++) {
			Sum_ele_ej += SNIIAllElements[ee] + SNIaAllElements[ee] + AGBAllElements[ee];
			Sum_ele_rem += SNIIUnProcessedElements[ee] + SNIaUnProcessedElements[ee] + AGBUnProcessedElements[ee];
			if (ee > 1) {
				Sum_met_ele_ej += SNIIAllElements[ee] + SNIaAllElements[ee] + AGBAllElements[ee];
				Sum_met_ele_rem += SNIIUnProcessedElements[ee] + SNIaUnProcessedElements[ee] + AGBUnProcessedElements[ee];
			}
#ifdef H2_AND_RINGS
			Sum_ele += Gal[p].DiskMassRings_elements[jj][ee];
			if (ee > 1) Sum_met_ele += Gal[p].DiskMassRings_elements[jj][ee];
#else
			Sum_ele += Gal[p].DiskMass_elements[ee];
			if (ee > 1) Sum_met_ele += Gal[p].DiskMass_elements[ee];
#endif
		}
#ifdef H2_AND_RINGS
		met_ratio = ((Gal[p].MetalsDiskMassRings[jj][0]+Gal[p].MetalsDiskMassRings[jj][1]+Gal[p].MetalsDiskMassRings[jj][2])*(1.e10/Hubble_h))/Sum_met_ele;
#else
		met_ratio = ((Gal[p].MetalsDiskMass[0]+Gal[p].MetalsDiskMass[1]+Gal[p].MetalsDiskMass[2])*(1.e10/Hubble_h))/Sum_met_ele;
#endif
		met_rem_ratio = ((SNIIUnProcessedMetals + SNIaUnProcessedMetals + AGBUnProcessedMetals)*(1.0e10/Hubble_h))/Sum_met_ele_rem;
		//if (met_rem_ratio > 1.3 || met_rem_ratio < 0.7) {
		if (met_ratio > 1.1 || met_ratio < 0.9) {
		//if (Sum_ele < 0.0) {
			printf("\nGal[p].SnapNum = %i | Gal[p].StellarMass = %e Msun:\n", Gal[p].SnapNum, (Gal[p].DiskMass+Gal[p].BulgeMass)*(1.e10/Hubble_h));
			printf("Total mass ejected = %e Msun | Sum(elements ejected) = %e Msun |         Ratio = %f\n", (SNIIEjectaMass + SNIaEjectaMass + AGBEjectaMass)*(1.0e10/Hubble_h), Sum_ele_ej, ((SNIIEjectaMass + SNIaEjectaMass + AGBEjectaMass)*(1.0e10/Hubble_h))/Sum_ele_ej);
			printf("Total metals ejected = %e Msun | Sum(metal elements ejected) = %e Msun | Ratio = %f\n", (SNIIAllMetals + SNIaAllMetals + AGBAllMetals)*(1.0e10/Hubble_h), Sum_met_ele_ej, ((SNIIAllMetals + SNIaAllMetals + AGBAllMetals)*(1.0e10/Hubble_h))/Sum_met_ele_ej);
			printf("Total mass removed = %e Msun | Sum(elements removed) = %e Msun |         Ratio = %f\n", (SNIIEjectaMass + SNIaEjectaMass + AGBEjectaMass)*(1.0e10/Hubble_h), Sum_ele_rem, ((SNIIEjectaMass + SNIaEjectaMass + AGBEjectaMass)*(1.0e10/Hubble_h))/Sum_ele_rem);
			printf("Total metals removed = %e Msun | Sum(metal elements removed) = %e Msun | Ratio = %f\n", (SNIIUnProcessedMetals + SNIaUnProcessedMetals + AGBUnProcessedMetals)*(1.0e10/Hubble_h), Sum_met_ele_rem, ((SNIIUnProcessedMetals + SNIaUnProcessedMetals + AGBUnProcessedMetals)*(1.0e10/Hubble_h))/Sum_met_ele_rem);
#ifdef H2_AND_RINGS
			printf("Total disc mass = %e Msun | Sum(disc elements) = %e Msun |               Ratio = %f\n", Gal[p].DiskMassRings[jj]*(1.e10/Hubble_h), Sum_ele, (Gal[p].DiskMassRings[jj]*(1.e10/Hubble_h))/Sum_ele);
			printf("Total disc metals = %e Msun | Sum(metal disc elements) = %e Msun |       Ratio = %f\n", (Gal[p].MetalsDiskMassRings[jj][0]+Gal[p].MetalsDiskMassRings[jj][1]+Gal[p].MetalsDiskMassRings[jj][2])*(1.e10/Hubble_h), Sum_met_ele, met_ratio);
#else
			printf("Total disc mass = %e Msun | Sum(disc elements) = %e Msun |               Ratio = %f\n", Gal[p].DiskMass*(1.e10/Hubble_h), Sum_ele, (Gal[p].DiskMass*(1.e10/Hubble_h))/Sum_ele);
			printf("Total disc metals = %e Msun | Sum(metal disc elements) = %e Msun |       Ratio = %f\n", (Gal[p].MetalsDiskMass[0]+Gal[p].MetalsDiskMass[1]+Gal[p].MetalsDiskMass[2])*(1.e10/Hubble_h), Sum_met_ele, met_ratio);
#endif
		}
		//##########
...
//New way to scale unprocessed components: Ensure that the total metals/each element removed doesn't exceed the total metals/each element currently in the stars:
		...
		if (UnprocessedMetals > Metals) {
			//NOTE: For those cases where Metals=0.0, then UnprocessedMetalsScaleFactor=0.0 here, as required:
			UnprocessedMetalsScaleFactor = Metals / UnprocessedMetals;
			//printf("Bulge: UnprocessedMetalsScaleFactor = %f | SNIIUnProcessedMetals = %e\n", UnprocessedMetalsScaleFactor, SNIIUnProcessedMetals);
			//Scale unprocessed material from each channel equally:
			SNIIUnProcessedMetals *= UnprocessedMetalsScaleFactor;
			SNIaUnProcessedMetals *= UnprocessedMetalsScaleFactor;
			AGBUnProcessedMetals *= UnprocessedMetalsScaleFactor;
		...
		for(ee=0;ee<NUM_ELEMENTS;ee++) {
			...
			if (UnProcessedElements > Element) {
				//NOTE: For those cases where Element=0.0, then UnprocessedElementsScaleFactor=0.0 here, as required:
				UnprocessedElementsScaleFactor = Element / UnProcessedElements;
				//printf("Bulge: ee = %i | UnprocessedElementsScaleFactor = %f\n", ee, UnprocessedElementsScaleFactor);
				SNIIUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
				SNIaUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
				AGBUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
			}
...
//New way to scale unprocessed components: Ensure that the total metals/each element removed doesn't exceed the total metals/each element currently in the stars:
		...
		if (UnprocessedMetals > Metals) {
			//NOTE: For those cases where Metals=0.0, then UnprocessedMetalsScaleFactor=0.0 here, as required:
			UnprocessedMetalsScaleFactor = Metals / UnprocessedMetals;
			//printf("ICM: UnprocessedMetalsScaleFactor = %f | SNIIUnProcessedMetals = %e\n", UnprocessedMetalsScaleFactor, SNIIUnProcessedMetals);
			//Scale unprocessed material from each channel equally:
			SNIIUnProcessedMetals *= UnprocessedMetalsScaleFactor;
			SNIaUnProcessedMetals *= UnprocessedMetalsScaleFactor;
			AGBUnProcessedMetals *= UnprocessedMetalsScaleFactor;
		...
		for(ee=0;ee<NUM_ELEMENTS;ee++) {
			...
			if (UnProcessedElements > Element) {
				//NOTE: For those cases where Element=0.0, then UnprocessedElementsScaleFactor=0.0 here, as required:
				UnprocessedElementsScaleFactor = Element / UnProcessedElements;
				//printf("ICM: UnprocessedElementsScaleFactor = %f\n", UnprocessedElementsScaleFactor);
				SNIIUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
				SNIaUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
				AGBUnProcessedElements[ee] *= UnprocessedElementsScaleFactor;
			}
...
//pre-calculations to speed up the code
  ...
  //SNII
  *SNIIEjectaMass = SFRxStep * NormSNIIMassEjecRate_actual; //ROB: Why is there no option for an unprocessed component to be added (for the P98 yield set) here? (17-05-22)
  ...
  *AGBRate = (SFRxStep_Phys/timestep_width) * NormAGBNum_actual; //This is the AGB rate in [1/(time code units)] in this timestep. So should divide by (UnitTime_in_s / SEC_PER_YEAR) in save.c to get "per year" before outputting).
  ...
  if (NormSNIIMassEjecRate_actual > 0.0) {
	  ...
	  if (SNII_met_rem_factor != SNII_met_rem_factor) printf("\n\nSNII_met_rem_factor = %f | *SNIIEjectaMass*(1.e10/Hubble_h) = %e | SNII_ele_sum = %e\n", SNII_met_rem_factor, *SNIIEjectaMass*(1.e10/Hubble_h), SNII_ele_sum);
  }
  if (NormSNIaMassEjecRate_actual > 0.0) {
	  SNIa_met_rem_factor = *SNIaEjectaMass*(1.e10/Hubble_h) / SNIa_ele_sum;
	  if (SNIa_met_rem_factor != SNIa_met_rem_factor) printf("\n\nSNIa_met_rem_factor = %f | *SNIaEjectaMass*(1.e10/Hubble_h) = %e | SNIa_ele_sum = %e\n", SNIa_met_rem_factor, *SNIaEjectaMass*(1.e10/Hubble_h), SNIa_ele_sum);
  }
  if (NormAGBMassEjecRate_actual > 0.0) {
	  AGB_met_rem_factor = *AGBEjectaMass*(1.e10/Hubble_h) / AGB_ele_sum;
	  if (AGB_met_rem_factor != AGB_met_rem_factor) printf("\n\nAGB_met_rem_factor = %f | *AGBEjectaMass*(1.e10/Hubble_h) = %e | AGB_ele_sum = %e\n", AGB_met_rem_factor, *AGBEjectaMass*(1.e10/Hubble_h), AGB_ele_sum);
  }
  met_rem_ratio = ((*SNIIUnProcessedMetals + *SNIaUnProcessedMetals + *AGBUnProcessedMetals)*(1.0e10/Hubble_h))/(SNII_met_ele_sum+SNIa_met_ele_sum+AGB_met_ele_sum);
  if (met_rem_ratio > 1.1 || met_rem_ratio < 0.9) {
	  printf("\n\n*SNIIEjectaMass = %e Msun | SNII_ele_sum = %e Msun | SNII_met_rem_factor = %f\n", *SNIIEjectaMass*(1.e10/Hubble_h), SNII_ele_sum, SNII_met_rem_factor);
	  printf("*SNIaEjectaMass = %e Msun | SNIa_ele_sum = %e Msun | SNIa_met_rem_factor = %f\n", *SNIaEjectaMass*(1.e10/Hubble_h), SNIa_ele_sum, SNIa_met_rem_factor);
	  printf("*AGBEjectaMass = %e Msun | AGB_ele_sum = %e Msun | AGB_met_rem_factor = %f\n", *AGBEjectaMass*(1.e10/Hubble_h), AGB_ele_sum, AGB_met_rem_factor);
	  printf("Uncorrected Sum(elements removed) = %e Msun | Corrected Sum(elements removed) = %e Msun\n", SNII_ele_sum+SNIa_ele_sum+AGB_ele_sum, (SNII_met_rem_factor*SNII_ele_sum)+(SNIa_met_rem_factor*SNIa_ele_sum)+(AGB_met_rem_factor*AGB_ele_sum));
  }
  for(ee=0;ee<NUM_ELEMENTS;ee++) {
