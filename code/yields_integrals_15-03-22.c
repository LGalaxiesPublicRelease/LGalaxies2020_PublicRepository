 ...
 int k; //Iterator for the L-Galaxies elements arrays [0 to 10]
			  ...
			  if (Mi_lower_AGB <= Mi_upper_AGB)
			      ...
			      for (j=Mi_lower_AGB;j<=Mi_upper_AGB;j++)
				{
				  if (j != Mi_lower_AGB && j != Mi_upper_AGB)
				    ...
				    if((AGBMasses[j+1]-AGBMasses[j]) * ((AGBYields[Zi_AGB][10][j] + AGBYields[Zi_AGB][10][j+1])/2.0) > 0.0)
				      {
				    ...
				    if((Mi_upper_actual-Mi_lower_actual) * ((AGBYields_lower_actual[10] + AGBYields_upper_actual[10])/2.0) > 0.0)
				      {
				      	printf("L2AGB: step = %i | SFHbin = %i | Zi = %i | AGBYields_lower_actual[10] = %f | AGBYields_upper_actual[10] = %f | NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10] = %f\n",
				      			(STEPS*snap)+step, i, Zi, AGBYields_lower_actual[10], AGBYields_upper_actual[10], NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10]);
				      }
#endif //INDIVIDUAL_ELEMENTS
				    }
				  else if (j == Mi_lower_AGB && j == Mi_upper_AGB && Mi_lower_actual >= AGB_MIN_MASS && Mi_lower_actual > AGB_MAX_MASS && Mi_upper_actual <= AGB_MAX_MASS) //NB: Mi_lower_actual cannot be below AGB_MIN_MASS
				    ...
				    if(NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10] > 0.0)
				      {
				      	printf("L3AGB: step = %i | SFHbin = %i | Zi = %i | NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10] = %f\n",
				      		   (STEPS*snap)+step, i, Zi, NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10]);
				      }
#endif //INDIVIDUAL_ELEMENTS
				    }
				    ...
				    if((AGB_MAX_MASS-Mi_lower_actual) * ((AGBYields_lower_actual[10] + AGBYields_upper_actual[10])/2.0) > 0.0)
				      {
				      	printf("L4AGB: step = %i | SFHbin = %i | Zi = %i | AGBYields_lower_actual[10] = %f | AGBYields_upper_actual[10] = %f | NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10] = %f\n",
				      			(STEPS*snap)+step, i, Zi, AGBYields_lower_actual[10], AGBYields_upper_actual[10], NormAGBYieldRate[(STEPS*snap)+step][i][Zi][10]);
				      }
#endif //INDIVIDUAL_ELEMENTS
				    }
  ...
  for(snap=0;snap<LastDarkMatterSnapShot;snap++) //LOOP OVER SNAPSHOTS
	...
	for(kk=0;kk<LIFETIME_Z_NUM;kk++) { //LOOP OVER METALLICITIES
	  FILE *SNIIyf;
	  char filename[100];
	  sprintf(filename, "./YieldTables/SNII_burst_yields_Z%.4f.txt", lifetimeMetallicities[kk]);
	...
	for(kk=0;kk<LIFETIME_Z_NUM;kk++) { //LOOP OVER METALLICITIES
	  FILE *SNIayf;
	  char filename[100];
	  sprintf(filename, "./YieldTables/SNIa_burst_yields_Z%.4f.txt", lifetimeMetallicities[kk]);
	...
	for(kk=0;kk<LIFETIME_Z_NUM;kk++) { //LOOP OVER METALLICITIES
	  FILE *AGByf;
	  char filename[100];
	  sprintf(filename, "./YieldTables/AGB_burst_yields_Z%.4f.txt", lifetimeMetallicities[kk]);
  ...
  printf("Burst yields files written.\n");
#endif //WRITE_BURST_YIELDS
  ...
  switch (channel_type)
  {
      ...
      else if (Mi_lower+1 == AGB_MASS_NUM) //This option added to prevent numerical integration beyond the maximum AGB mass bin with AGBMasses[Mi_lower+1]. ROB (18-06-21)
	{
	  ...
	  *TotalMetals_lower_actual = AGBTotalMetals[Zi][AGB_MASS_NUM-1];
	  ...
	  if(Yields_lower_actual[10] > 0.0) {printf("YLA2: Mi_lower+1 = %i | AGBMasses[Mi_lower] = %f | AGBMasses[Mi_lower+1] = %f | Mi_upper_actual = %f | Yields_lower_actual[10] = %f\n",
			  Mi_lower+1, AGBMasses[Mi_lower], AGBMasses[Mi_lower+1], Mi_upper_actual, Yields_lower_actual[10]);}
#endif //INDIVIDUAL_ELEMENTS
	}
      else
      	{
      	  ...
      	  *TotalMetals_lower_actual = AGBTotalMetals[Zi][Mi_lower] + ((AGBTotalMetals[Zi][Mi_lower+1]-AGBTotalMetals[Zi][Mi_lower]) * ((Mi_lower_actual-AGBMasses[Mi_lower])/(AGBMasses[Mi_lower+1]-AGBMasses[Mi_lower])));
      	  ...
      	  if(Yields_lower_actual[10] > 0.0) {printf("YLA3: Mi_lower+1 = %i | AGBMasses[Mi_lower] = %f | AGBMasses[Mi_lower+1] = %f | Mi_upper_actual = %f | Yields_lower_actual[10] = %f\n",
      			  Mi_lower+1, AGBMasses[Mi_lower], AGBMasses[Mi_lower+1], Mi_upper_actual, Yields_lower_actual[10]);}
      #endif //INDIVIDUAL_ELEMENTS
      	}
