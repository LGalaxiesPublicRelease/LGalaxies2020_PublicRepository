...
void do_AGN_heating(double dt, int ngal)
{
    ...
    double c2=pow2(SPEEDOFLIGHT/UnitVelocity_in_cm_per_s); // c^2 - Speed of light squared in code units
...
void cool_gas_onto_galaxy(int p, double dt)
{
  ...
  if(Mcool > 0.0)
...
#ifdef DETAILED_DUST
      for (jj=0; jj<RNUM; jj++) {
     	fractionCloudsRings[jj] = fractionRings[jj]*Gal[p].H2fractionRings[jj];
     	fractionDiffRings[jj] = fractionRings[jj]*(1.-Gal[p].H2fractionRings[jj]);
      }
      transfer_material_with_rings(p, "ColdGas", p, "HotGas", fractionRings, fractionCloudsRings, fractionDiffRings, "cool_gas_onto_galaxy", __LINE__);
      //N.B. When dust is resolved into rings too, we'll need to pass Gal[p].H2fractionRings here.
#else
      ...
      /*if(Gal[p].ReheatedGas>0.)
	{
      ...
      transfer_material_with_rings(p, "ColdGas", p, "ReheatedGas", fractionRings, fractionCloudsRings, fractionDiffRings, "cool_gas_onto_galaxy", __LINE__);
#else
...
#else
#ifdef DETAILED_DUST
      transfer_material(p, "ColdGas", p, "HotGas", fraction, fraction*Gal[p].H2fraction, fraction*(1.-Gal[p].H2fraction), "cool_gas_onto_galaxy", __LINE__);
#else
      ...
      /*if(Gal[p].ReheatedGas>0.)
    	{
    	  ...
    	  fraction=Mcool/Gal[p].ReheatedGas;
#ifdef DETAILED_DUST
      	  transfer_material(p, "ColdGas", p, "ReheatedGas", fraction, fraction*Gal[p].H2fraction, fraction*(1.-Gal[p].H2fraction), "cool_gas_onto_galaxy", __LINE__);
#else
