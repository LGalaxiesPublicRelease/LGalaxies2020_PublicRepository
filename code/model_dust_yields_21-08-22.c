...
//**************************************************
	...
	if ((Gal[p].MetalsColdGas[0]+Gal[p].MetalsColdGas[1]+Gal[p].MetalsColdGas[2]) > 0.0) {
		...
		*/
		...
		//As (a) the total ColdGas mass is the sum of the ColdGas mass in each ring, and (b) the total SN rate is the sum of the SN rates in each ring, then we should just be able to sum the tdes from each ring to get the total tdes: (08-02-22):
		/*if (R_SN > 0.0)
			Gal[p].t_des += tdes; //Note, this will be the total destruction timescale across the whole snapshot. (09-02-22)*/
#else //H2_AND_RINGS
		...
		*/
		...
		if (Gal[p].HotGas > 0.0 && HotGasDust > 0.0) {
			...
			HotGasDensity = (Gal[p].HotGas*UnitMass_in_g) / ((4./3.) * PI * pow3(Gal[p].Rvir*UnitLength_in_cm)); //[g/cm^3]
			Delta_grainRadius = (sputConst * (HotGasDensity/PROTONMASS) * (1./(pow(sputT0/Tvir,sputOmega)+1.))) * 1.e5 * (dt * UnitTime_in_s); //[micron] //Eqn.22 Triani+20 (except here, Delta_grainRadius is positive) //Multiple of 1.0e5 here is to convert cm to microns. //HotGasDensity in g/cm^3, VIrialTemp in K
			//printf("\nTvir = %e | HotGasDensity = %e | Delta_grainRadius = %f\n", Tvir, HotGasDensity, Delta_grainRadius); //if (p == 0 && Gal[p].SnapNum < 16)
  ...
  //(09-03-22): partition_gas_and_dust_elements() has now been moved to the end of update_h2fraction(), so that the cloud and diffuse gas compoennts are always in sync with the current H2 fraction.
...
if (((Gal[p].MetalsColdGas[0]+Gal[p].MetalsColdGas[1]+Gal[p].MetalsColdGas[2])>0.0) && (Gal[p].ColdGas > 0.0)) {
        ...
        */
		...
		if (isnan(Gal[p].f_cmax[j][Cb_NUM])) {Gal[p].f_cmax[j][Cb_NUM] = 1.;} //f_Cb_max could legitimately be -nan, if the mass of C in ring j is 0.0.
		if (f_Cb_max < 0.0 || f_Cb_max > 1.0)
			printf("***** WARNING: f_Cb_max has unrealistic value: f_Cb_max = %f\n", f_Cb_max);
		...
		if (isnan(Gal[p].f_cmax[j][O_NUM])) {Gal[p].f_cmax[j][O_NUM] = 1.;} //f_O_max could legitimately be -nan, if the mass of O in ring j is 0.0.
		if (f_O_max < 0.0 || f_O_max > 1.0)
			printf("***** WARNING: f_O_max has unrealistic value: f_O_max = %f\n", f_O_max);
#else
#ifndef MAINELEMENTS
