...
void disrupt(int p, int centralgal)
{
  ...
  if (rho_cen > rho_sat)
  {
    ...
    for (jj=0;jj<RNUM;jj++) {
    	fractionRings[jj]=1.;
#ifdef DETAILED_DUST
    	fractionCloudsRings[jj]=0.; //0. used for the cloud and diffuse transfer fractions here, as there are neither clouds nor diffuse components in either ICM or BulgeMass.
    	fractionDiffRings[jj]=0.;
#endif //DETAILED_DUST
    }
#ifdef DETAILED_DUST
    transfer_material_with_rings(centralgal,"ICM",p,"DiskMass",fractionRings,fractionCloudsRings,fractionDiffRings,"disrupt.c", __LINE__);
    transfer_material_with_rings(centralgal,"ICM",p,"BulgeMass",fractionRings,fractionCloudsRings,fractionDiffRings,"disrupt.c", __LINE__);
#else
...
#else //H2_AND_RINGS
#ifdef DETAILED_DUST
    transfer_material(centralgal,"ICM",p,"BulgeMass",1.,0.,0.,"disrupt.c", __LINE__);
    transfer_material(centralgal,"ICM",p,"DiskMass",1.,0.,0.,"disrupt.c", __LINE__);
#else
...
#endif //DETAILED_DUST
#endif //H2_AND_RINGS
    //ROB: Why isn't any remaining ICM stellar material transferred from p to centralgal here too? I think it is gradually stripped like HotGas in model_stripping.c. (25-11-21)

#ifdef H2_AND_RINGS
#ifdef DETAILED_DUST
    transfer_material_with_rings(centralgal,"HotGas",p,"ColdGas",fractionRings,fractionRings,fractionRings,"disrupt.c", __LINE__); //fractionRings (i.e. 1.) used for cloud/diff gas here, as all this material should also be disrupted from the ColdGas to the HotGas.
    //Update dust destruction rate:
	for(ee=0;ee<NUM_ELEMENTS;ee++) {
#ifdef DUST_HOTGAS
		//Dust is transferred to HotGas undestroyed:
		Gal[p].DustColdGasRates[4] += 0.0; //(Gal[p].DustColdGasDiff_elements[ee]*(1.-fractionRings) + Gal[p].DustColdGasClouds_elements[ee]*1.-fractionRings))/(deltaT * UnitTime_in_years);
#else //DUST_HOTGAS
		//All dust destroyed as galaxy disrupts:
		Gal[p].DustColdGasRates[4] += (Gal[p].DustColdGasDiff_elements[ee]*1. + Gal[p].DustColdGasClouds_elements[ee]*1.)/(deltaT * UnitTime_in_years);
		/*for (jj=0;jj<RNUM;jj++) {
			Gal[p].DustColdGasRates[4] += (Gal[p].DustColdGasDiffRings_elements[jj][ee]*1. + Gal[p].DustColdGasCloudsRings_elements[jj][ee]*1.)/(deltaT * UnitTime_in_years);
		}*/
#endif //DUST_HOTGAS
	}
#else //DETAILED_DUST
...
#else //H2_AND_RINGS
#ifdef DETAILED_DUST
    transfer_material(centralgal,"HotGas",p,"ColdGas",1.,1.,1.,"disrupt.c", __LINE__); //ROB: 25-11-21: When adding dust to transfer_material() need to decide if dust can be transferred to HotGas undestroyed here.
    //Update dust destruction rate:
    for(ee=0;ee<NUM_ELEMENTS;ee++) {
#ifdef DUST_HOTGAS
		//Dust is transferred to HotGas undestroyed:
		Gal[p].DustColdGasRates[4] += 0.0;
#else //DUST_HOTGAS
		//All dust destroyed as galaxy disrupts:
    	Gal[p].DustColdGasRates[4] += (Gal[p].DustColdGasDiff_elements[ee]*1. + Gal[p].DustColdGasClouds_elements[ee]*1.)/(deltaT * UnitTime_in_years);
#endif //DUST_HOTGAS
...
/*#ifdef DETAILED_DUST //ROB: Dust destruction is now done in transfer_material(), and the update to the dust destruction rate is done just above here. (26-11-21)
	  transfer_dust_to_hot(p,1.);
	  mass_checks("transfer_dust_to_hot disrupt",p);
#endif*/

#ifdef DETAILED_DUST
    //transfer_material(centralgal,"HotGas",p,"HotGas",1.,0.,0.,"disrupt.c", __LINE__);
    transfer_material(centralgal,"HotGas",p,"HotGas",1.,0.0,1.0,"disrupt.c", __LINE__); //If DUST_HOTGAS is on, transfer 100% of HotGas dust as if it were only diffuse.
#else
