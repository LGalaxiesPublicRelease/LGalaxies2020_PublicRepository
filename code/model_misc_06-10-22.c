  ...
  else if (strcmp(cq,"EjectedMass")==0)
  {
      ...
      for(ee=0;ee<NUM_ELEMENTS;ee++)
      {
    	  Yield[ee] = Gal[q].EjectedMass_elements[ee]*fraction;
      	  ...
      	  //Dust_clouds[ee] = Gal[q].EjectedMass_elements[ee]*fractionClouds; //This is in case we decide to keep dust in the HotGas later on.
      	  //Dust_diffuse[ee] = Gal[q].EjectedMass_elements[ee]*fractionDiff; //This is in case we decide to keep dust in the HotGas later on.
#endif
      }
#endif
  }
  ...
  else if (strcmp(cq,"ExcessMass")==0)
  {
      ...
      for(ee=0;ee<NUM_ELEMENTS;ee++)
      {
    	  Yield[ee] = Gal[q].ExcessMass_elements[ee]*fraction;
      	  ...
      	  //Dust_clouds[ee] = Gal[q].ExcessMass_elements[ee]*fractionClouds; //This is in case we decide to keep dust in the HotGas later on.
      	  //Dust_diffuse[ee] = Gal[q].ExcessMass_elements[ee]*fractionDiff; //This is in case we decide to keep dust in the HotGas later on.
#endif
      }
#endif
  }
...
//Variables for LUMINOSITIES
  ...
  //Initialize arrays to contain mass to transfer:
   ...
   for(ee=0;ee<NUM_ELEMENTS;ee++) {
	...
	/*//if (checka == 1.) {
		printf("***** Start of transfer_material_with_rings():\n");
		printf("Snapnum %i | From: %s | To: %s | Call function: %s\n", Gal[p].SnapNum, cq, cp, call_function);
		...
		for (int itjj=0;itjj<RNUM;itjj++)
			printf("fractionCloudsRings[%i] = %f\n", itjj, fractionCloudsRings[itjj]);
	//}*/
 ...
 */
  ...
  //Therefore, it only needs to be added here if cp = ColdGas.
  ...
  else if (strcmp(cq,"ExcessMass")==0)
  {
   ...
   //mass_checks(q,"model_misc.c",__LINE__);
	...
	/*//if (checka == 1.) {
		printf("***** End of transfer_material_with_rings():\n");
		printf("Snapnum %i\n", Gal[p].SnapNum);
		for (int itee=0; itee<NUM_ELEMENTS; itee++)
			printf("Gal[p].DustColdGasClouds_elements[%i] = %f\n", itee, Gal[p].DustColdGasClouds_elements[itee]);
		for (int itjj=0;itjj<RNUM;itjj++)
