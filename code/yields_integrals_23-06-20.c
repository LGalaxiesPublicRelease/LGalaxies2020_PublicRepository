...
void integrate_yields()
{
  ...
  int Zi_pick, i_pick;
  Zi_pick = 0; //3 //Choose which of the discrete lifetime metallicities to assume when calculating the SN-II yields: [0.0004, 0.004, 0.008, 0.02, 0.05, 1]
			      ...
			      for (j=Mi_lower_SNII;j<=Mi_upper_SNII;j++)
				{
			      ...
			      if (SNIIMasses[j] <= SNIA_MAX_MASS) //For mass range where both SN-II and SN-Ia progenitors are possible
				      ...
				      if (j != Mi_lower_SNII && j != Mi_upper_SNII) //If mass bin j is NEITHER the lowest NOR highest mass bin to be integrated over.
					{
					  ...
					  ML = SNIIMasses[j];
					  MU = SNIIMasses[j+1];
					  if (mb == width_in_timesteps && MU == Mi_upper_actual && MU < Mi_lower_lastTS[i][Zi][0]) MU = Mi_lower_lastTS[i][Zi][0]; //only for last mass bin integrated over
					  if (MU > Mi_lower_lastTS[i][Zi][0] && Mi_lower_lastTS[i][Zi][0] > 0.0) MU = Mi_lower_lastTS[i][Zi][0]; // && Mi_lower_lastTS[i][Zi][0] > 0.0
					  if (MU < ML) ML = MU;
					  ...
					  //if ((STEPS*snap)+step >= 14 && (STEPS*snap)+step <= 17 && Zi == Zi_pick && i == i_pick) {printf("%i: L1a: SFHBin %i MiniBin %i | DeltaTmb = %e yr | lowM = %e | highM = %e | DeltaM = %e | Chab(lowM) = %e | Chab(highM) = %e | SNII_Rate = %e\n", (STEPS*snap)+step, i, mb, t_upper-t_lower, ML, MU, MU-ML, Chabrier_IMF(ML), Chabrier_IMF(MU), SNII_Rate[(STEPS*snap)+step][Zi]);}
					  //if (i == 0) NormSNIINum[(STEPS*snap)+step][Zi] += (1.0-A_FACTOR) * (MU-ML) * (Chabrier_IMF(ML) + Chabrier_IMF(MU))/2.0; //Number of SNe-II exploding in this timestep from that minibin [units: # / Msun]
					  /*SNII_Rate[(STEPS*snap)+step][Zi] += (1.0-A_FACTOR) * (SNIIMasses[j+1]-SNIIMasses[j]) * ((Chabrier_IMF(SNIIMasses[j])*TheSFH[i]/First_SFH_bin_width) + (Chabrier_IMF(SNIIMasses[j+1])*TheSFH[i]/First_SFH_bin_width))/2.0;
					  NormSNIINum[(STEPS*snap)+step][i][Zi] += (1.0-A_FACTOR) * (SNIIMasses[j+1]-SNIIMasses[j]) * (Chabrier_IMF(SNIIMasses[j]) + Chabrier_IMF(SNIIMasses[j+1]))/2.0; //Number of SNe-II exploding in this timestep from that minibin [units: # / Msun]
					  //if ((STEPS*snap)+step >= 14 && (STEPS*snap)+step <= 17 && Zi == Zi_pick && i == i_pick) {printf("%i: L1a: SFHBin %i MiniBin %i | DeltaTmb = %e yr | lowM = %e | highM = %e | DeltaM = %e | Chab(lowM) = %e | Chab(highM) = %e | SNII_Rate = %e\n", (STEPS*snap)+step, i, mb, t_upper-t_lower, SNIIMasses[j], SNIIMasses[j+1], SNIIMasses[j+1]-SNIIMasses[j], Chabrier_IMF(SNIIMasses[j]), Chabrier_IMF(SNIIMasses[j+1]), SNII_Rate[(STEPS*snap)+step][Zi]);}*/
					  NormSNIIMassEjecRate[(STEPS*snap)+step][i][Zi] += (1.0-A_FACTOR) * ((SNIIMasses[j+1]-SNIIMasses[j]) * ((SNIIEjectedMasses[Zi_SNII][j] + SNIIEjectedMasses[Zi_SNII][j+1])/2.0)); //This is a unitless quantity, which must be multiplied by SFR (i.e. M_starsFormedInSFHBin*dt_SFH, in model_yields.c) to get units of [Msun/yr], and then by dt (i.e. timestep width, also in model_yields.c) to get units of [Msun]. (22-05-20) //NO! --> //IN [MSun/yr] //The rate of ejection of mass from a 1 Msun stellar population.
					  NormSNIIMetalEjecRate[(STEPS*snap)+step][i][Zi] += (1.0-A_FACTOR) * (SNIIMasses[j+1]-SNIIMasses[j]) * ((SNIITotalMetals[Zi_SNII][j] + SNIITotalMetals[Zi_SNII][j+1])/2.0); //This is a unitless quantity, which must be multiplied by SFR (in model_yields.c) to get units of [Msun/yr], and then by dt (also in model_yields.c) to get units of [Msun]. (22-05-20) //NO! --> //IN [MSun/yr] //The rate of ejection of 'newly synthesised' metals from a 1 Msun stellar population.
#ifdef INDIVIDUAL_ELEMENTS
				      ...
				      else if (j == Mi_upper_SNII && j != Mi_lower_SNII) //If mass bin j IS NOT the lowest but IS the highest mass bin to be integrated over.
					{
					  ...
					  ML = SNIIMasses[j];
					  MU = Mi_upper_actual;
					  if (mb == width_in_timesteps && MU == Mi_upper_actual && MU < Mi_lower_lastTS[i][Zi][0]) MU = Mi_lower_lastTS[i][Zi][0];
					  if (MU > Mi_lower_lastTS[i][Zi][0] && Mi_lower_lastTS[i][Zi][0] > 0.0) MU = Mi_lower_lastTS[i][Zi][0];
					  if (MU < ML) ML = MU;
					  SNII_Rate[(STEPS*snap)+step][Zi] += (1.0-A_FACTOR) * (MU-ML) * ((Chabrier_IMF(ML)*TheSFH[i]/First_SFH_bin_width) + (Chabrier_IMF(MU)*TheSFH[i]/First_SFH_bin_width))/2.0;
					  NormSNIINum[(STEPS*snap)+step][i][Zi] += (1.0-A_FACTOR) * (MU-ML) * (Chabrier_IMF(ML) + Chabrier_IMF(MU))/2.0;
