    ...
    if (Gal[Gal[p].CentralGal].Type == 1) {      /* If type 1, or type 2 orbiting type 1 near type 0 */
		if (FateOfSatellitesGas == 0) {
			...
			for(ee=0;ee<NUM_ELEMENTS;ee++) {
				...
				Gal[p].DustEjectedMassRates += 0.0; //No dust is destroyed just by moving to the EjectedMass when DUST_EJECTEDMASS is on.
#else //DUST_EJECTEDMASS
				Gal[p].DustHotGasRates[3] += (Gal[p].DustHotGas_elements[ee]*fraction)/(deltaT * UnitTime_in_years); //27-02-23: Currently, all dust transferred to the Ejecta component is assumed to be fully and instantaneously destroyed.
